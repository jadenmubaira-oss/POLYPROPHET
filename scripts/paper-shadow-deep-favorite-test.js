...
async function main() {
    ...
    const thresholds = [15, 20, 25, 30, 35, 40];
    const results = {};
    ...
    for (const threshold of thresholds) {
        ...
        for (const [cycleKey, cycle] of cycles.entries()) {
            ...
            // Find first tick where absMoveBps >= threshold AND it's in the first 5 minutes
            const triggerTick = cycle.ticks.find(t => t.bps >= threshold && t.secondsIntoEpoch <= 300);
            if (!triggerTick) continue;

            const ask = triggerTick.dir === 'UP' ? triggerTick.bestAsk : triggerTick.noBestAsk;
            
            // Try different ask limits
    ...
    // Fetch missing settlements for the minimum threshold (15) to cover all others
    const opps15 = results[15][0.95].opportunities;
    const nowSec = Math.floor(Date.now() / 1000);
    
    for (const opp of opps15) {
    ...
    for (const threshold of thresholds) {
        for (const limit of [0.70, 0.75, 0.80, 0.85, 0.90, 0.95]) {
            ...
            for (const opp of results[threshold][limit].opportunities) {
                ...
                const stake = Math.min(bankroll * 0.25, 50);
            ...
            if (tradesExecuted > 0) {
                ...
                console.log(`Thr ${threshold} bps, Ask <= ${limit}: Opps: ${results[threshold][limit].opportunities.length}, Exec: ${tradesExecuted}, W: ${wins}, L: ${losses}, WR: ${wr}, Bankroll: $${bankroll.toFixed(2)}`);
            }
        }
    }
}
