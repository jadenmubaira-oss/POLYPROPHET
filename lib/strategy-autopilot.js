...
class StrategyAutopilot {
  ...
  recordError(error, trigger) {
    ...
    this.persist();
    const isMissingArtifact = this.state.lastError.includes('AUTOPILOT_ARTIFACT_MISSING');
    this.diagnostic(isMissingArtifact ? 'STRATEGY_AUTOPILOT_SKIPPED' : 'STRATEGY_AUTOPILOT_ERROR', { trigger, error: this.state.lastError });
    if (!isMissingArtifact && this.telegram?.notifyError) this.telegram.notifyError('strategy-autopilot', error);
  }
  ...
  async runPipeline(trigger, startedAt) {
    ...
    const searchEnv = {
      ...
      HARD_ENTRY_PRICE_CAP: process.env.HARD_ENTRY_PRICE_CAP || '0.45',
