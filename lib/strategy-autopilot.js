...
class StrategyAutopilot {
    ...
    evaluateFreshAuditCandidate(currentAudit, sourceAudit, artifact) {
        ...
        if ((candidateSummary.triggered || 0) < Math.max(this.minPruneEvents, 8)) issues.push('CANDIDATE_SAMPLE_SMALL');
        if ((currentSummary.triggered || 0) < Math.max(this.minPruneEvents, 8)) issues.push('CURRENT_SAMPLE_SMALL');
        ...
        if (candidateSummary.avgPnlPerShare !== null && currentSummary.avgPnlPerShare !== null && candidateSummary.avgPnlPerShare <= currentSummary.avgPnlPerShare) issues.push('DOES_NOT_BEAT_CURRENT_AVG_PNL');
        const reviewable = candidateSummary.rules > 0 && !issues.includes('DOES_NOT_BEAT_CURRENT_AVG_PNL') && !issues.includes('DOES_NOT_BEAT_CURRENT_WIN_RATE');
        return {
