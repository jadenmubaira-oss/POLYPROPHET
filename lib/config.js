...
// Epoch 3 V2: bounded-aggressive stake fractions for the live-edge 0.97 ask regime.
// Operator-accepted max-profit profile (Opus v3): k=0.60 bootstrap/growth, taper at scale.
// $5-$15: 0.60, $15-$50: 0.60, $50-$200: 0.50, $200+: 0.40
const epoch3TieredSizing = parseBool(process.env.EPOCH3_TIERED_SIZING, true);
const defaultStakeFraction = epoch3TieredSizing
    ? (startingBalance <= 15 ? 0.60 : startingBalance <= 50 ? 0.60 : startingBalance <= 200 ? 0.50 : 0.40)
...
const allowMicro5m = parseBool(
    process.env.MICRO_BANKROLL_ALLOW_5M ?? process.env.ALLOW_MICRO_5M ?? process.env.ALLOW_MICRO_TIMEFRAME_OVERRIDE,
    epoch3TieredSizing
);
const allowMicro4h = parseBool(
    process.env.MICRO_BANKROLL_ALLOW_4H ?? process.env.ALLOW_MICRO_4H ?? process.env.ALLOW_MICRO_TIMEFRAME_OVERRIDE,
    false
);
...
const CONFIG = {
    ...
    MICRO_BANKROLL_ALLOW_MPC_OVERRIDE: allowMicroMpcOverride,
    MICRO_BANKROLL_ALLOW_5M: allowMicro5m,
    MICRO_BANKROLL_ALLOW_4H: allowMicro4h,
    ...
    POLYMARKET_FORCE_DERIVE_CREDS: String(process.env.POLYMARKET_FORCE_DERIVE_CREDS || 'false').toLowerCase() === 'true',
    POLYMARKET_ORDER_AUTH_DERIVE: String(process.env.POLYMARKET_ORDER_AUTH_DERIVE || 'true').toLowerCase() !== 'false',
    POLYMARKET_RAW_POLY1271_SIGNING: String(process.env.POLYMARKET_RAW_POLY1271_SIGNING || 'false').toLowerCase() === 'true',
    ...
    CLOB_ORDER_TYPE: String(process.env.CLOB_ORDER_TYPE || 'FAK').trim().toUpperCase(),

    // Assets & timeframes
    ASSETS: parseList(process.env.ASSETS, defaultAssets),
    TIMEFRAMES: [
        { key: '5m', seconds: 300, label: '5-Minute', enabled: parseBool(process.env.TIMEFRAME_5M_ENABLED ?? process.env.MULTIFRAME_5M_ENABLED, allowMicro5m), minBankroll: parseNumber(process.env.TIMEFRAME_5M_MIN_BANKROLL, epoch3TieredSizing ? 3 : 50) },
    ...
    RISK: {
        ...
        kellyMaxFraction: parseNumber(process.env.KELLY_MAX_FRACTION, 0.60),
        kellyFraction: parseNumber(process.env.KELLY_FRACTION, 0.60),
        kellyMinPWin: 0.55,
        globalStopLoss: 0.20,
        ...
        maxMatchedSharesMultiplier: parseNumber(process.env.MAX_MATCHED_SHARES_MULTIPLIER, 1.05),
        ...
        slippagePct: parseNumber(process.env.SLIPPAGE_PCT, 0),
        ...
        minNetEdgeRoi: parseNumber(process.env.MIN_NET_EDGE_ROI, 0.015),
        ...
        skipRecoveryQueueGate: parseBool(process.env.SKIP_RECOVERY_QUEUE_GATE, false),
        ...
        orderAuthProofMode: String(process.env.ORDER_AUTH_PROOF_MODE || 'false').toLowerCase() === 'true',
        orderAuthProofMinNetEdgeRoi: parseNumber(process.env.ORDER_AUTH_PROOF_MIN_NET_EDGE_ROI, 0),
        requireRealOrderBook: parseBool(process.env.REQUIRE_REAL_ORDERBOOK, true),
        hardEntryPriceCap: parseNumber(process.env.HARD_ENTRY_PRICE_CAP, 0.45),
        orderbookDepthGuardEnabled: parseBool(process.env.ORDERBOOK_DEPTH_GUARD_ENABLED, true),
        orderbookDepthGuardSafetyMult: parseNumber(process.env.ORDERBOOK_DEPTH_GUARD_SAFETY_MULT, 1.05),
        ...
        preResolutionMinBid: parseNumber(process.env.PRE_RESOLUTION_MIN_BID, 0.99),
...
if (!['FAK', 'FOK', 'GTC', 'GTD'].includes(CONFIG.CLOB_ORDER_TYPE)) {
    console.warn(`⚠️ CLOB_ORDER_TYPE=${CONFIG.CLOB_ORDER_TYPE} is not recognized; supported values are FAK, FOK, GTC, GTD.`);
}
