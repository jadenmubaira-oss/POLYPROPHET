...
dotenv.config(envFilePath ? { path: envFilePath } : undefined);
 ...
 const forcedMaxGlobalTradesPerCycle = microBankrollDeployProfile ? 1 : Math.max(1, requestedMaxGlobalTradesPerCycle);
 ...
 const CONFIG = {
    ...
    TIMEFRAMES: [
        { key: '5m', seconds: 300, label: '5-Minute', enabled: microBankrollDeployProfile ? true : parseBool(process.env.TIMEFRAME_5M_ENABLED ?? process.env.MULTIFRAME_5M_ENABLED, false), minBankroll: microBankrollDeployProfile ? 2 : parseNumber(process.env.TIMEFRAME_5M_MIN_BANKROLL, 50) },
        { key: '15m', seconds: 900, label: '15-Minute', enabled: parseBool(process.env.TIMEFRAME_15M_ENABLED, true), minBankroll: microBankrollDeployProfile ? 2 : parseNumber(process.env.TIMEFRAME_15M_MIN_BANKROLL, 5) },
        { key: '4h', seconds: 14400, label: '4-Hour', enabled: microBankrollDeployProfile ? false : parseBool(process.env.MULTIFRAME_4H_ENABLED ?? process.env.ENABLE_4H_TRADING, false), minBankroll: parseNumber(process.env.TIMEFRAME_4H_MIN_BANKROLL, 10) }
    ],
...
if (microBankrollDeployProfile) {
    console.warn('⚠️ Micro-bankroll deploy profile active: forcing 15m ultra-tight posture, enabling 5m overlay, disabling 4h, and capping MPC at 1.');
}
