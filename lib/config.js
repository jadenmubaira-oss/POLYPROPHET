...
dotenv.config(envFilePath ? { path: envFilePath } : undefined);
 ...
 const parsedMaxGlobalTradesPerCycle = parseInt(process.env.MAX_GLOBAL_TRADES_PER_CYCLE || '7', 10);
 const requestedMaxGlobalTradesPerCycle = Number.isFinite(parsedMaxGlobalTradesPerCycle) ? parsedMaxGlobalTradesPerCycle : 7;
 const forcedMaxGlobalTradesPerCycle = Math.max(7, requestedMaxGlobalTradesPerCycle);
...
if (forcedMaxGlobalTradesPerCycle !== requestedMaxGlobalTradesPerCycle) {
    console.warn(`⚠️ MAX_GLOBAL_TRADES_PER_CYCLE=${requestedMaxGlobalTradesPerCycle} overridden to ${forcedMaxGlobalTradesPerCycle} to force MPC=7+ runtime posture.`);
}
