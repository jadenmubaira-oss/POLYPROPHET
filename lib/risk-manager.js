...
class RiskManager {
    ...
    _tieredStakeFraction(bankroll, configuredBase) {
        ...
        if (stakingStrategy === 'target_ladder') {
            ...
            const t1Frac = Number(process.env.STAKING_TIER1_FRACTION || 0.60);
            const t2Max = Number(process.env.STAKING_TIER2_MAX || 500);
            const t2Frac = Number(process.env.STAKING_TIER2_FRACTION || 0.30);
            const t3Frac = Number(process.env.STAKING_TIER3_FRACTION || 0.10);
