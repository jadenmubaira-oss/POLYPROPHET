...
class RiskManager {
    ...
    _getTierProfile(bankroll) {
        ...
        // Epoch 3 V2: Tiered aggression per DEFINITIVE PLAN Phase F
        // $5-$15: SF=0.40, MPC=1 (sequential high-conviction)
        // $15-$50: SF=0.35, MPC=2 (allow 2 concurrent)
        // $50-$200: SF=0.30, MPC=3 (standard Kelly)
        // $200+: SF=0.25, MPC=3-5 (start profit-taking)
        if (bankroll < 15) {
            // When EPOCH3_TIERED_SIZING=true, config.js sets SF=0.40 which flows here.
            // When false, config.js sets SF=0.15. Math.min respects operator overrides.
            const sf = Math.min(configuredStakeFraction, 0.45);
            return { maxPerCycle: Math.min(configuredMaxPerCycle, 1), stakeFraction: sf, label: 'BOOTSTRAP' };
        }
        if (bankroll < 50) {
            const sf = Math.min(configuredStakeFraction, 0.35);
            return { maxPerCycle: Math.min(configuredMaxPerCycle, 2), stakeFraction: sf, label: 'GROWTH' };
        }
        if (bankroll < 200) {
            const sf = Math.min(configuredStakeFraction, 0.30);
            return { maxPerCycle: Math.min(configuredMaxPerCycle, 3), stakeFraction: sf, label: 'ACCELERATE' };
        }
        return { maxPerCycle: configuredMaxPerCycle, stakeFraction: Math.min(configuredStakeFraction, 0.25), label: 'PRESERVE' };
    }
