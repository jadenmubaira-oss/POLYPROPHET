...
class RiskManager {
    ...
    _getTierProfile(bankroll) {
        const configuredMaxPerCycle = Math.max(1, parseInt(CONFIG?.RISK?.maxGlobalTradesPerCycle || '5', 10));
        ...
        // Epoch 3 V2 (Opus v3 max-profit): tier caps for the live-edge 0.97 ask regime.
        // $5-$15: SF=0.60, MPC=1 (sequential high-conviction bootstrap)
        // $15-$50: SF=0.60, MPC=2 (compound faster while still slot-limited)
        // $50-$200: SF=0.50, MPC=3 (depth-guarded acceleration)
        // $200+: SF=0.40, MPC=3-5 (capacity-limited growth)
        if (bankroll < 15) {
            // When EPOCH3_TIERED_SIZING=true, config.js sets SF=0.60 which flows here.
            // When false, config.js sets SF=0.15. Math.min respects operator overrides.
            const sf = Math.min(configuredStakeFraction, 0.60);
            return { maxPerCycle: Math.min(configuredMaxPerCycle, 1), stakeFraction: sf, label: 'BOOTSTRAP' };
        }
        if (bankroll < 50) {
            const sf = Math.min(configuredStakeFraction, 0.60);
            return { maxPerCycle: Math.min(configuredMaxPerCycle, 2), stakeFraction: sf, label: 'GROWTH' };
        }
        if (bankroll < 200) {
            const sf = Math.min(configuredStakeFraction, 0.50);
            return { maxPerCycle: Math.min(configuredMaxPerCycle, 3), stakeFraction: sf, label: 'ACCELERATE' };
        }
        return { maxPerCycle: configuredMaxPerCycle, stakeFraction: Math.min(configuredStakeFraction, 0.40), label: 'PRESERVE' };
    }
