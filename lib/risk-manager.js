...
class RiskManager {
    ...
    _getTierProfile(bankroll) {
        const configuredMaxPerCycle = Math.max(1, parseInt(CONFIG?.RISK?.maxGlobalTradesPerCycle || '2', 10));
        const configuredStakeFractionRaw = Number(CONFIG?.RISK?.stakeFraction);
        const configuredStakeFraction = Number.isFinite(configuredStakeFractionRaw)
            ? Math.min(1, Math.max(0, configuredStakeFractionRaw))
            : 0.15;
        // Tiered stake fraction for micro-bankroll escape velocity
        const tieredStakeFraction = this._tieredStakeFraction(bankroll, configuredStakeFraction);
        // Micro bankroll MPC: allow env override via ALLOW_MICRO_MPC_OVERRIDE
        const safeMPC = bankroll < 20 ? Math.min(configuredMaxPerCycle, Math.max(2, configuredMaxPerCycle)) : configuredMaxPerCycle;
        if (bankroll < 10) return { maxPerCycle: safeMPC, stakeFraction: tieredStakeFraction, label: 'BOOTSTRAP' };
        if (bankroll < 50) return { maxPerCycle: safeMPC, stakeFraction: tieredStakeFraction, label: 'GROWTH' };
        if (bankroll < 200) return { maxPerCycle: safeMPC, stakeFraction: tieredStakeFraction, label: 'ACCELERATE' };
        return { maxPerCycle: configuredMaxPerCycle, stakeFraction: tieredStakeFraction, label: 'PRESERVE' };
    }

    _tieredStakeFraction(bankroll, configuredBase) {
        const stakingStrategy = process.env.STAKING_STRATEGY;

        // Phase 3 fixed stake (e.g., STAKING_STRATEGY=fixed, PHASE3_STAKE_FRACTION=0.45)
        if (stakingStrategy === 'fixed') {
            const fixedFrac = Number(process.env.PHASE3_STAKE_FRACTION || configuredBase);
            return fixedFrac > 0 && fixedFrac <= 1 ? fixedFrac : configuredBase;
        }

        // Target-ladder staking (from epoch3 comprehensive strategy set)
        if (stakingStrategy === 'target_ladder') {
            const t1Max = Number(process.env.STAKING_TIER1_MAX || 50);
            const t1Frac = Number(process.env.STAKING_TIER1_FRACTION || 0.55);
            const t2Max = Number(process.env.STAKING_TIER2_MAX || 500);
            const t2Frac = Number(process.env.STAKING_TIER2_FRACTION || 0.35);
            const t3Frac = Number(process.env.STAKING_TIER3_FRACTION || 0.15);
            if (bankroll < t1Max) return t1Frac;
            if (bankroll < t2Max) return t2Frac;
            return t3Frac;
        }

        // Legacy aggressive tiered sizing for micro-bankroll escape
        if (bankroll < 6) return Math.max(configuredBase, 0.55);
        if (bankroll < 10) return Math.max(configuredBase, 0.45);
        if (bankroll < 18) return Math.max(configuredBase, 0.38);
        if (bankroll < 35) return Math.max(configuredBase, 0.32);
        if (bankroll < 75) return Math.max(configuredBase, 0.26);
        if (bankroll < 200) return Math.max(configuredBase, 0.22);
        if (bankroll < 500) return Math.max(configuredBase, 0.18);
    ...
    calculateSize(candidate, context = {}) {
        ...
        // Peak drawdown brake
        const drawdownBrake = this.getPeakDrawdownBrakeState(bankroll);
        stakeFraction = drawdownBrake.effectiveStakeFraction;
    ...
    resetMonitoringBaseline(balance, options = {}) {
        ...
        const paused = this.tradingPaused;
        ...
        this.tradingPaused = preservePause ? paused : false;

        this.lastRebaseAt = Date.now();
        this.monitoringBaselineAt = this.lastRebaseAt;
        return true;
    }
    ...
    exportState() {
        return {
            ...
            monitoringBaselineAt: this.monitoringBaselineAt || null
        };
    }
    ...
    importState(state = {}) {
        ...
        if (Number.isFinite(Number(state.monitoringBaselineAt))) this.monitoringBaselineAt = Number(state.monitoringBaselineAt);
    }

    getStatus() {
        this._resetDayIfNeeded();
        const wr = this.totalTrades > 0 ? (this.totalWins / this.totalTrades * 100).toFixed(1) : 'N/A';
        const drawdownBrake = this.getPeakDrawdownBrakeState(this.bankroll);
        return {
            bankroll: this.bankroll,
            dayStartBalance: this.dayStartBalance,
            peakBalance: this.peakBalance,
    ...
    getPeakDrawdownBrakeState(bankroll = this.bankroll) {
        const currentBankroll = Number.isFinite(Number(bankroll)) ? Number(bankroll) : Number(this.bankroll || 0);
        const peakBalance = Number.isFinite(Number(this.peakBalance)) ? Number(this.peakBalance) : currentBankroll;
        const tier = this._getTierProfile(currentBankroll);
        const nominalStakeFraction = Number(tier?.stakeFraction || 0);
        const thresholdPct = Number(CONFIG.RISK.peakDrawdownBrakePct || 0);
        const minBankroll = Number(CONFIG.RISK.peakDrawdownBrakeMinBankroll || 0);
        const drawdownPct = peakBalance > 0 ? Math.max(0, (peakBalance - currentBankroll) / peakBalance) : 0;
        ...
        return {
            ...
            thresholdPct,
            minBankroll,
            nominalStakeFraction,
            effectiveStakeFraction: active ? Math.min(nominalStakeFraction, 0.12) : nominalStakeFraction
        };
    }
