...
class RiskManager {
    ...
    _getTierProfile(bankroll) {
        ...
        const configuredStakeFraction = Number.isFinite(configuredStakeFractionRaw)
            ...
            : 0.15;
        if (bankroll < 10) return { maxPerCycle: configuredMaxPerCycle, stakeFraction: configuredStakeFraction, label: 'BOOTSTRAP' };
        if (bankroll < 50) return { maxPerCycle: configuredMaxPerCycle, stakeFraction: configuredStakeFraction, label: 'GROWTH' };
        if (bankroll < 200) return { maxPerCycle: configuredMaxPerCycle, stakeFraction: configuredStakeFraction, label: 'ACCELERATE' };
