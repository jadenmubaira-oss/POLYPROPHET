...
class RiskManager {
    ...
    _getTierProfile(bankroll) {
        ...
        const configuredStakeFraction = Number.isFinite(configuredStakeFractionRaw)
            ...
            : 0.15;
        // SAFETY CAP: micro bankroll (<$20) hard-caps MPC at 2 regardless of env var.
        // This prevents the #1 historical failure mode: Render env MPC=7 overriding
        // safe simulation defaults and guaranteeing bust at small bankrolls.
        const safeMPC = bankroll < 20 ? Math.min(configuredMaxPerCycle, 2) : configuredMaxPerCycle;
        if (bankroll < 10) return { maxPerCycle: safeMPC, stakeFraction: configuredStakeFraction, label: 'BOOTSTRAP' };
        if (bankroll < 50) return { maxPerCycle: safeMPC, stakeFraction: configuredStakeFraction, label: 'GROWTH' };
        if (bankroll < 200) return { maxPerCycle: safeMPC, stakeFraction: configuredStakeFraction, label: 'ACCELERATE' };
        return { maxPerCycle: configuredMaxPerCycle, stakeFraction: configuredStakeFraction, label: 'PRESERVE' };
    }
