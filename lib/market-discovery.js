...
async function fetchCLOBBook(tokenId) {
     try {
        const requestOptions = { useProxy: !!CONFIG.CLOB_FORCE_PROXY };
        const [bookResult, buyPriceResult] = await Promise.allSettled([
            fetchJSON(`${CONFIG.CLOB_API}/book?token_id=${tokenId}`, 10000, requestOptions),
            fetchJSON(`${CONFIG.CLOB_API}/price?token_id=${tokenId}&side=BUY`, 10000, requestOptions)
        ]);
