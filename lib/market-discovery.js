...
async function fetchCLOBBook(tokenId) {
     try {
        ...
        const forceProxy = !!(CONFIG.CLOB_FORCE_PROXY && proxyAgent);
        const directAllowed = !forceProxy;
        ...
        let [bookResult, buyPriceResult] = directAllowed
            ? await fetchPair(false)
            : await fetchPair(true);
        ...
        const needProxyRetry = !forceProxy
            && proxyAgent
            && !isUsableBook(bookPayload)
            && !isUsablePrice(pricePayload);
