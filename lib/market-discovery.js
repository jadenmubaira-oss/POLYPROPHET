...
async function fetchCLOBBook(tokenId) {
     try {
        ...
        // Try direct first (CLOB read-only endpoints are not geoblocked)
        let [bookResult, buyPriceResult] = await Promise.allSettled([
            fetchJSON(bookUrl, 10000),
            fetchJSON(priceUrl, 10000)
        ]);

        // If both direct calls failed and proxy is available, retry via proxy
        if (bookResult.status !== 'fulfilled' && buyPriceResult.status !== 'fulfilled' && proxyAgent) {
            [bookResult, buyPriceResult] = await Promise.allSettled([
                fetchJSON(bookUrl, 10000, { useProxy: true }),
                fetchJSON(priceUrl, 10000, { useProxy: true })
            ]);
        }

        const book = bookResult.status === 'fulfilled' ? bookResult.value : null;
        const buyPricePayload = buyPriceResult.status === 'fulfilled' ? buyPriceResult.value : null;
...
async function discoverMarket(asset, tf, nowSec) {
    ...
    const yesPrice = yesBook?.buyPrice ?? yesBook?.bestAsk ?? yesBook?.midpoint ?? null;
    const noPrice = noBook?.buyPrice ?? noBook?.bestAsk ?? noBook?.midpoint ?? null;
