...
// V1 collateral (USDC.e) â€” legacy, still used for on-chain balance fallback
...
// V2 collateral (Polymarket USD / pUSD) â€” primary after CTF Exchange V2 cutover (Apr 28 2026)
...
function redactClobDiagnosticText(value) {
  ...
  return String(text || "")
    .replace(/0x[a-fA-F0-9]{64,}/g, (m) => `${m.slice(0, 10)}â€¦${m.slice(-6)}`)
    .replace(/Bearer\s+[A-Za-z0-9._~+\-/=]+/gi, "Bearer [REDACTED]")
    .replace(/"(POLY_API_KEY|POLY_SECRET|POLY_PASSPHRASE|POLY_SIGNATURE)"\s*:\s*"[^"]*"/gi, '"$1":"[REDACTED]"')
    .replace(/"(POLY_ADDRESS)"\s*:\s*"0x[a-fA-F0-9]{40}"/gi, (_m, key) => `"${key}":"[ADDRESS]"`)
...
console.error = (...args) => {
  originalConsoleError(
    ...args.map((arg) => {
      if (typeof arg === "string") return redactClobDiagnosticText(arg);
      if (arg instanceof Error) {
        return redactClobDiagnosticText(arg.stack || arg.message || "");
      }
      if (arg && typeof arg === "object") return redactClobDiagnosticText(arg);
      return arg;
    }),
  );
};
...
let createL1Headers = null;
let ethers = null;
let axios = null;
let clobSdkAxios = null;
...
// EPOCH 2: V2 SDK migration â€” dual-path loader with positional-arg adapter
...
try {
  const {
    ClobClient: V2ClobClient,
    OrderType: V2OrderType,
    createL1Headers: V2CreateL1Headers,
  } = require("@polymarket/clob-client-v2");
  if (V2ClobClient) {
    if (V2OrderType) OrderType = V2OrderType;
    if (typeof V2CreateL1Headers === "function") createL1Headers = V2CreateL1Headers;
    ...
    console.log("âœ… Polymarket CLOB V2 SDK loaded (positional adapter active)");
  }
} catch (e) {
  console.log("âš ï¸ @polymarket/clob-client-v2 not installed â€” trying V1 fallback...");
  try {
    ClobClient = require("@polymarket/clob-client").ClobClient;
    console.log("âš ï¸ V1 SDK fallback loaded â€” WILL STOP WORKING after V2 cutover (Apr 28 2026)");
  } catch (e2) {
    console.log("âš ï¸ Neither V1 nor V2 CLOB SDK installed â€” LIVE trading disabled");
  }
}
...
try {
  ...
  console.log("âš ï¸ ethers not installed â€” wallet operations disabled");
}

try {
  axios = require("axios");
} catch (e) {
  console.log("âš ï¸ axios not installed â€” CLOB proxy routing disabled");
}

try {
  clobSdkAxios = require("@polymarket/clob-client-v2/node_modules/axios");
} catch (e) {
  try {
    const clobSdkMain = require.resolve("@polymarket/clob-client-v2");
    const clobSdkRoot = path.resolve(path.dirname(clobSdkMain), "..");
    clobSdkAxios = require(path.join(clobSdkRoot, "node_modules", "axios"));
  } catch (e2) {
    clobSdkAxios = null;
  }
}
...
try {
  ...
  console.log(
    "âš ï¸ @polymarket/builder-relayer-client not installed â€” proxy auto-redemption disabled",
  );
}
...
try {
  ...
  console.log(
    "âš ï¸ builder-relayer proxy helpers unavailable â€” proxy auto-redemption disabled",
  );
}
...
try {
  ...
  console.log(
    "âš ï¸ builder-relayer config helpers unavailable â€” proxy auto-redemption disabled",
  );
}
...
try {
  ...
  console.log(
    "âš ï¸ @polymarket/builder-signing-sdk not installed â€” builder-auth relayer submits disabled",
  );
}
...
const clobRoutedAxiosInstances = new WeakSet();
...
async function withClobAuthContext(signatureType, funderAddress, fn, authAddress = null) {
  const previous = activeClobAuthContext;
  activeClobAuthContext = { signatureType, funderAddress, authAddress };
  try {
    return await fn();
  } finally {
...
function getAxiosInstanceName(instance) {
  if (instance === clobSdkAxios) return "clob-client-v2";
  if (instance === axios) return "root";
  return "unknown";
}

function installClobAxiosRouting() {
  const axiosInstances = [axios, clobSdkAxios].filter(
    (instance) =>
      !!instance &&
      !!instance.interceptors?.request &&
      !clobRoutedAxiosInstances.has(instance),
  );
  if (axiosInstances.length === 0) {
    clobAxiosRoutingInstalled =
      clobAxiosRoutingInstalled || clobRoutedAxiosInstances.size > 0;
    return;
  }
  ...
  // Do NOT set axios.defaults.httpsAgent â€” it causes "Converting circular structure
  ...
  for (const instance of axiosInstances) {
    const axiosInstanceName = getAxiosInstanceName(instance);
    ...
    const authAddress = activeClobAuthContext?.authAddress || null;
    ...
    let polyAddressOverridden = false;
    const headerOverrideAddress = authAddress || null;
    if (
      headerOverrideAddress &&
      headers &&
      ethers?.utils?.isAddress &&
      ethers.utils.isAddress(headerOverrideAddress)
    ) {
      if (typeof headers.set === "function") headers.set("POLY_ADDRESS", ethers.utils.getAddress(headerOverrideAddress));
      else headers.POLY_ADDRESS = ethers.utils.getAddress(headerOverrideAddress);
      polyAddressOverridden = true;
    }
    ...
    const routeEvent = {
      ...
      polyAddressOverridden,
      axiosInstance: axiosInstanceName,
      agent: "none",
      proxied: false,
    };
    if (isReadOnly) {
    ...
    clobRoutedAxiosInstances.add(instance);
  }
  ...
  if (CONFIG.PROXY_URL && CONFIG.CLOB_FORCE_PROXY && clobProxyAgent) {
    ...
    console.log(
      " CLOB proxy available but CLOB_FORCE_PROXY not set â€” all direct",
    );
  }
}
...
class PolymarketCLOB {
  ...
  loadWallet() {
    if (!CONFIG.POLYMARKET_PRIVATE_KEY) {
      console.log(" No POLYMARKET_PRIVATE_KEY â€” wallet not loaded");
      return false;
    }
    if (!ethers) {
      console.log(" ethers not available â€” cannot load wallet");
      return false;
    }
  ...
  async _deriveApiCredsFor(sigType, funder, label) {
    try {
      ...
      const raw = await Promise.race([
        withClobAuthContext(sigType, funder, async () => {
          if (typeof tmp.createApiKey === "function") {
            try {
              const created = await tmp.createApiKey();
              const createdCreds = this._extractApiCreds(created);
              if (createdCreds) return createdCreds;
            } catch (err) {
              console.log(
                ` Create creds failed (${err?.response?.status || err?.status || "ERR"}); trying derive`,
              );
            }
          }
          if (typeof createL1Headers === "function" && (clobSdkAxios || axios)) {
            try {
              const headers = await createL1Headers(this.wallet, POLY_CHAIN_ID);
              const http = clobSdkAxios || axios;
              const derived = await http
                .get(`${host}/auth/derive-api-key`, { headers, proxy: false })
                .then((res) => res.data);
              const derivedCreds = this._extractApiCreds(derived);
              if (derivedCreds) return derivedCreds;
            } catch (err) {
              console.log(
                ` HTTP derive failed (${err?.response?.status || err?.status || "ERR"}); trying SDK derive`,
              );
            }
          }
          if (typeof tmp.deriveApiKey === "function") {
            return await tmp.deriveApiKey();
          }
          return await tmp.createOrDeriveApiKey();
        }),
        new Promise((_, rej) =>
        ...
        for (const [sigType, funder, label] of attempts) {
          ...
          if (creds) {
            ...
            this._lastDerivedClobCreds = {
              ...
              POLYMARKET_API_AUTH_ADDRESS: creds.authAddress || null,
            };
            console.log(
  ...
  async rederiveCreds() {
    ...
    CONFIG.POLYMARKET_SECRET = "";
    CONFIG.POLYMARKET_PASSPHRASE = "";
    this._clobApiCredsAuthAddress = null;
    this._deriveCredsPromise = null;
  ...
  async _ensureBuilderCreds() {
    ...
    this._deriveBuilderCredsPromise = (async () => {
      try {
        const host = "https://clob.polymarket.com";
        const sigType = normalizeSignatureType(CONFIG.POLYMARKET_SIGNATURE_TYPE) ?? 0;
        const funder =
          sigType === 1 || sigType === 2
            ? getConfiguredFunderAddress() || this.wallet.address
            : this.wallet.address;
        const creds = {
        ...
        const raw = await Promise.race([
          withClobAuthContext(sigType, funder, () => tmp.createBuilderApiKey()),
          new Promise((_, rej) =>
            setTimeout(
  ...
  async _probeTradeClients(opts = {}) {
    ...
    const getCandidateAuthAddress = (signatureType, funderAddress) => {
      if (
        signatureType === 1 &&
        this._isAddress(this._clobApiCredsAuthAddress) &&
        String(this._clobApiCredsAuthAddress).toLowerCase() ===
          String(funderAddress || "").toLowerCase()
      ) {
        return ethers.utils.getAddress(this._clobApiCredsAuthAddress);
      }
      return null;
    };
    ...
    const candidates = [
      {
        ...
        source: "eoa",
        authAddress: getCandidateAuthAddress(0, signerAddress),
    ...
    const candidateKeys = new Set(
      candidates.map((candidate) =>
        `${candidate.signatureType}:${String(candidate.funderAddress || "").toLowerCase()}`,
      ),
    );
    const addProxyCandidate = (signatureType, funderAddress, source, deployed) => {
      if (!this._isAddress(funderAddress)) return;
      const key = `${signatureType}:${String(funderAddress).toLowerCase()}`;
      ...
      candidates.push({
        ...
        authAddress: getCandidateAuthAddress(signatureType, funderAddress),
        client: new ClobClient(
          host,
          POLY_CHAIN_ID,
          this.wallet,
    ...
    if (this._isAddress(primarySigType1Funder)) {
      ...
      const primaryDeployed =
        primarySigType1Funder !== signerAddress
          ? await this._isContractDeployed(primarySigType1Funder)
          : null;
      addProxyCandidate(3, primarySigType1Funder, `${primarySource}_deposit_wallet`, primaryDeployed);
      addProxyCandidate(1, primarySigType1Funder, primarySource, primaryDeployed);
      addProxyCandidate(2, primarySigType1Funder, `${primarySource}_safe`, primaryDeployed);
    }
    ...
    if (
      ...
      addProxyCandidate(3, proxyResolution.address, `${proxyResolution.source}_deposit_wallet`, proxyResolution.deployed);
      addProxyCandidate(1, proxyResolution.address, proxyResolution.source, proxyResolution.deployed);
      addProxyCandidate(2, proxyResolution.address, `${proxyResolution.source}_safe`, proxyResolution.deployed);
    }
    ...
    if (getConfiguredFunderAddress()) {
      try {
        ...
        if (derivedCheck?.ok && this._isAddress(derivedCheck.address)) {
          const alreadyFunder = candidates.some(
            (c) =>
          ...
          if (!alreadyFunder) {
            console.log(
              `âš ï¸ Auto-derived proxy ${derivedCheck.address} differs from configured POLYMARKET_ADDRESS ${getConfiguredFunderAddress()} â€” adding as extra candidate`,
            );
          }
    ...
    const probe = async (candidate) => {
      const runBalanceProbe = async (probeCandidate) => {
        const updated = await withClobAuthContext(
          probeCandidate.signatureType,
          probeCandidate.funderAddress,
          () =>
            probeCandidate.client
              .updateBalanceAllowance({ asset_type: "COLLATERAL" })
              .catch((e) => ({ error: e?.message || String(e) })),
          probeCandidate.authAddress || null,
        );
        ...
        return { updated, balanceAllowance };
      };
      ...
      let { updated, balanceAllowance } = await runBalanceProbe(candidate);
      if (
        updated &&
      ...
      const isInvalidApiKey =
        (updated &&
          typeof updated === "object" &&
          Object.prototype.hasOwnProperty.call(updated, "error") &&
          String(updated.error || "").toLowerCase().includes("invalid api key")) ||
        (balanceAllowance &&
          typeof balanceAllowance === "object" &&
          Object.prototype.hasOwnProperty.call(balanceAllowance, "error") &&
          String(balanceAllowance.error || "").toLowerCase().includes("invalid api key"));
      const balanceRawBeforeRetry =
        String(balanceAllowance?.balance ?? "").trim() || null;
      if (
        candidate.signatureType === 1 &&
        CONFIG.POLYMARKET_AUTO_DERIVE_CREDS &&
        (isInvalidApiKey || balanceRawBeforeRetry === "0")
      ) {
        const derivedCreds = await this._deriveApiCredsFor(
          candidate.signatureType,
          candidate.funderAddress,
          balanceRawBeforeRetry === "0"
            ? "probe-zero-balance-funder-retry"
            : "probe-invalid-api-key",
        );
        if (derivedCreds) {
          ...
          candidate.authAddress = derivedCreds.authAddress || null;
          out.client = candidate.client;
          out.refreshError = null;
          ({ updated, balanceAllowance } = await runBalanceProbe(candidate));
          out.refreshed = !(
            updated &&
            typeof updated === "object" &&
            Object.prototype.hasOwnProperty.call(updated, "error")
          );
          if (!out.refreshed) out.refreshError = String(updated.error);
        }
      }
    ...
    const failedProxyProbes = probes.filter(
      (p) =>
        (p.signatureType === 1 || p.signatureType === 2) &&
        (p.status === 401 || !p.ok),
    );
    for (const failedProbe of failedProxyProbes) {
      ...
      if (!this._isAddress(proxyAddr)) {
        console.log(
          ` Proxy probe ${failedProbe.funderAddress?.slice(0, 10)} ${failedProbe.status || "failed"} â€” invalid funder address`,
        );
        continue;
      }
      ...
      if (verifiedProxyBalance <= 0) {
        console.log(
          ` Proxy probe ${proxyAddr.slice(0, 10)} ${failedProbe.status || "failed"} â€” no verified collateral balance at that address`,
        );
        continue;
      }

      failedProbe.onChainCollateralBalance = verifiedProxyBalance;
      console.log(
        ` Proxy probe ${proxyAddr.slice(0, 10)} ${failedProbe.status || "failed"} â€” on-chain collateral $${verifiedProxyBalance.toFixed(2)} found, but CLOB balance/allowance is not trade-ready`,
      );
      failedProbe.ok = false;
      failedProbe.error =
        failedProbe.error ||
        `CLOB balance/allowance not ready despite on-chain collateral $${verifiedProxyBalance.toFixed(6)}; verify Polymarket profile/funder deposit flow`;
    }
      ...
      const result = await withClobAuthContext(
        ...
        async () => {
          ...
          if (response && response.orderID) {
            console.log(
              `âœ… CLOB ORDER PLACED: ${response.orderID} | sigType=${candidate.signatureType} | ${side} ${shares} @ ${(price * 100).toFixed(1)}Â¢`,
            );
            ...
            for (let attempt = 1; attempt <= 3; attempt++) {
              ...
              try {
                ...
                if (orderStatus) {
                  ...
                  if (matchedShares > 0) {
                    console.log(
                      `âœ… Order MATCHED: ${matchedShares.toFixed(4)} shares`,
                    );
                    break;
                  } else if (
  ...
  async getTradeReadyClient(opts = {}) {
    ...
    try {
      ...
      if (selKey !== this._lastSelectionKey) {
        ...
        console.log(
          `ðŸ” CLOB: sigType=${sigType} ${closedBlocks ? "CLOSED_ONLY" : ok ? "READY" : "NOT_READY"} balance=${Number.isFinite(selectedBalance) ? selectedBalance : "n/a"}`,
        );
      }
  ...
  async placeOrder(tokenId, price, shares, side = "BUY", opts = {}) {
    ...
    let attemptOrder = probeState.ok
      ? this._buildAttemptOrder(
          probeState.probes,
          probeState.preferredSigType,
          probeState.closedOnly,
        )
      : [];
    if (attemptOrder.length === 0 && opts.bypassReadinessForProof) {
      const proofCandidates = probeState.probes.filter(
        (probe) =>
          !probeState.closedOnly &&
          !!probe.client &&
          Number(probe.onChainCollateralBalance || 0) > 0 &&
          (probe.signatureType === probeState.preferredSigType ||
            probe.signatureType === 1),
      );
      const preferredProof = proofCandidates.find(
        (probe) => probe.signatureType === probeState.preferredSigType,
      );
      attemptOrder = preferredProof
        ? [preferredProof, ...proofCandidates.filter((probe) => probe !== preferredProof)]
        : proofCandidates;
    }
    if (!probeState.ok && attemptOrder.length === 0) {
