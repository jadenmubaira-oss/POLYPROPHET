...
const CTF_EXCHANGE_V2_ADDRESS = "0xE111180000d2663C0091e4f400237545B87B996B";
...
function buildClobFailureDiagnostic(errorOrResponse) {
  ...
  const data = response?.data ?? response?.body ?? response?.error ?? null;
  return {
    status: response?.status || response?.statusCode || null,
    statusText: response?.statusText || null,
    code: errorOrResponse?.code || null,
    message: redactClobDiagnosticText(errorOrResponse?.message || response?.message || response?.error || ""),
...
function withClobCallTimeout(fn, timeoutMs, label) {
  let timer = null;
  return Promise.race([
    Promise.resolve().then(fn),
    new Promise((_, reject) => {
      timer = setTimeout(
        () => reject(new Error(label || `CLOB_TIMEOUT_${timeoutMs}ms`)),
        timeoutMs,
      );
    }),
  ]).finally(() => {
...
function roundNormal(value, decimals) {
...
function uint16Hex(value) {
  ...
  if (!Number.isInteger(n) || n < 0 || n > 0xffff) {
    throw new Error(`Invalid uint16 length: ${value}`);
  }
  return n.toString(16).padStart(4, "0");
}

function generateClobSafeSalt() {
...
function normalizeSignatureType(value) {
  ...
  if (![0, 1, 2, 3].includes(parsed)) return null;
  return parsed;
}

function buildDepositWalletSigner(eoaSigner, depositWalletAddress) {
  // For SignatureTypeV2.POLY_1271 (3), the order signer is the deposit wallet.
  // The CLOB L2 auth header `POLY_ADDRESS` is derived from `signer.getAddress()`, so we must
  // present the deposit wallet address there, while still signing typed data with the EOA key.
  const deposit = ethers.utils.getAddress(depositWalletAddress);
  return {
    // Ethers TypedDataSigner interface
    getAddress: async () => deposit,
    _signTypedData: (domain, types, value) => eoaSigner._signTypedData(domain, types, value),

    // Keep parity with Wallet API surface when available
    signMessage: (...args) => (typeof eoaSigner.signMessage === "function" ? eoaSigner.signMessage(...args) : Promise.reject(new Error("signMessage unavailable"))),
    signTransaction: (...args) => (typeof eoaSigner.signTransaction === "function" ? eoaSigner.signTransaction(...args) : Promise.reject(new Error("signTransaction unavailable"))),

    provider: eoaSigner.provider,
  };
}
...
class PolymarketCLOB {
  ...
  async ensureCreds() {
    ...
    this._deriveCredsPromise = (async () => {
      try {
        ...
        const configuredFunder = getConfiguredFunderAddress();
        let proxyAddr = configuredFunder;
        if (!proxyAddr && [1, 3].includes(prefSigType)) {
          const derivedProxy = await this._deriveProxyWalletAddress(60000).catch(() => null);
          if (derivedProxy?.ok && this._isAddress(derivedProxy.address)) {
            proxyAddr = derivedProxy.address;
          }
        }

        const tryDerive = async (sigType, funder, label) => {
          return await this._deriveApiCredsFor(sigType, funder, label);
        };

        // Try preferred sigType first, then compatible fallbacks.
        const attempts = [];
        const addAttempt = (sigType, funder, label) => {
          if (!Number.isFinite(Number(sigType)) || !funder) return;
          const normalizedSigType = normalizeSignatureType(sigType);
          if (normalizedSigType === null) return;
          const key = `${normalizedSigType}:${String(funder).toLowerCase()}`;
          if (attempts.some((a) => `${a[0]}:${String(a[1]).toLowerCase()}` === key)) return;
          attempts.push([normalizedSigType, funder, label]);
        };

        addAttempt(prefSigType, proxyAddr || signerAddr, `preferred-sigType${prefSigType}`);
        addAttempt(0, signerAddr, "eoa-sigType0");
        addAttempt(1, proxyAddr, "proxy-sigType1-fallback");
        addAttempt(3, proxyAddr, "proxy-sigType3-fallback");

        for (const [sigType, funder, label] of attempts) {
          const creds = await tryDerive(sigType, funder, label);
          if (creds) {
            this._applyClobCreds(creds, label);
            console.log(
  ...
  _applyClobCreds(creds, label = "derived") {
  ...
  async _deriveApiCredsFor(sigType, funder, label, timeoutMs = 20000) {
    if (!ClobClient || !this.wallet) return null;
    try {
      const normalizedSigType = normalizeSignatureType(sigType);
      if (normalizedSigType === null || !this._isAddress(funder)) return null;
      const host = "https://clob.polymarket.com";
      console.log(
        ` Deriving creds: sigType=${normalizedSigType} funder=${funder?.slice(0, 10)}... (${label})`,
      );

      const signerForClient =
        normalizedSigType === 3
          ? buildDepositWalletSigner(this.wallet, funder)
          : this.wallet;
      const tmp = new ClobClient({
        host,
        chain: POLY_CHAIN_ID,
        signer: signerForClient,
        signatureType: normalizedSigType,
        // clob-client-v2 uses `funder` to enable deposit-wallet flow.
        funder,
        funderAddress: funder,
      });
      const deriveFn = async () => {
        if (typeof tmp.createApiKey === "function") {
          try {
            const created = await tmp.createApiKey();
            if (this._extractApiCreds(created)) return created;
          } catch (createErr) {
            console.log(
              ` Create API key (${label}) failed; trying derive: ${createErr.message || String(createErr)}`,
            );
          }
        }
        if (typeof tmp.deriveApiKey === "function") return await tmp.deriveApiKey();
        if (typeof tmp.createOrDeriveApiKey === "function") return await tmp.createOrDeriveApiKey();
        return null;
      };
      const raw = await Promise.race([
        deriveFn(),
        new Promise((_, rej) =>
          setTimeout(() => rej(new Error(`DERIVE_TIMEOUT_${Math.round(timeoutMs / 1000)}s`)), timeoutMs),
        ),
      ]);
      return this._extractApiCreds(raw);
    } catch (e) {
  ...
  async _refreshOrderSignerCredsForCandidate(candidate, reason = "order-auth") {
    if (!CONFIG.POLYMARKET_AUTO_DERIVE_CREDS) {
      return { ok: false, reason: "POLYMARKET_AUTO_DERIVE_CREDS disabled" };
    }
    if (!CONFIG.POLYMARKET_ORDER_AUTH_DERIVE) {
      return { ok: false, reason: "POLYMARKET_ORDER_AUTH_DERIVE disabled" };
    }
    if (!candidate || !this.wallet) {
      return { ok: false, reason: "Missing candidate or wallet" };
    }

    const attempts = [];
    const addAttempt = (sigType, funder, label) => {
      const normalizedSigType = normalizeSignatureType(sigType);
      if (normalizedSigType === null || !this._isAddress(funder)) return;
      const key = `${normalizedSigType}:${String(funder).toLowerCase()}`;
      if (attempts.some((a) => a.key === key)) return;
      attempts.push({ key, sigType: normalizedSigType, funder, label });
    };

    addAttempt(candidate.signatureType, candidate.funderAddress, `${reason}-candidate`);
    addAttempt(0, this.wallet.address, `${reason}-eoa`);

    for (const attempt of attempts) {
      const creds = await this._deriveApiCredsFor(
        attempt.sigType,
        attempt.funder,
        attempt.label,
      );
      if (!creds) continue;
      this._applyClobCreds(creds, attempt.label);

      candidate.client = new ClobClient({
        host: "https://clob.polymarket.com",
        chain: POLY_CHAIN_ID,
        signer:
          candidate.signatureType === 3
            ? buildDepositWalletSigner(this.wallet, candidate.funderAddress)
            : this.wallet,
        creds,
        signatureType: candidate.signatureType,
        // clob-client-v2 uses `funder` to enable deposit-wallet flow.
        funder: candidate.funderAddress,
        funderAddress: candidate.funderAddress,
      });
      candidate.credsSource = attempt.label;
      return { ok: true, source: attempt.label };
    }

    return { ok: false, reason: "Order-signer credential rederive failed" };
  }
  ...
  async _probeTradeClients(opts = {}) {
    ...
    const configuredIsSigner = !!(
      configuredFunderAddress &&
      signerAddress &&
      String(configuredFunderAddress).toLowerCase() === String(signerAddress).toLowerCase()
    );
    const pureEoaPreferred = preferredSigType === 0 && configuredIsSigner;
    ...
    const addCandidate = async (
      ...
      const normalizedSigType = normalizeSignatureType(signatureType);
      if (normalizedSigType === null || !this._isAddress(funderAddress)) return;
      const key = `${normalizedSigType}:${String(funderAddress).toLowerCase()}`;
      if (
        candidates.some(
          (candidate) =>
            `${candidate.signatureType}:${String(candidate.funderAddress).toLowerCase()}` === key,
        )
      ) {
        return;
      }
      candidates.push({
        ...
        client: new ClobClient({
          ...
          signer:
            normalizedSigType === 3
              ? buildDepositWalletSigner(this.wallet, funderAddress)
              : this.wallet,
          creds,
          signatureType: normalizedSigType,
          // clob-client-v2 expects `funder` (deposit wallet) for deposit-wallet flow.
          // Keep `funderAddress` too for backwards compatibility and our own diagnostics.
    ...
    await addCandidate(0, signerAddress, "eoa");

    if (!pureEoaPreferred && this._isAddress(primarySigType1Funder)) {
      ...
      const deployed =
        primarySigType1Funder !== signerAddress
          ...
          : null;
      await addCandidate(1, primarySigType1Funder, primarySource, deployed);
      if (preferredSigType !== 0 && preferredSigType !== 1) {
        await addCandidate(
          preferredSigType,
          primarySigType1Funder,
          `${primarySource}_sigType${preferredSigType}`,
          deployed,
        );
      }
    }
    ...
    if (
      ...
      await addCandidate(
        1,
        proxyResolution.address,
        proxyResolution.source,
        proxyResolution.deployed,
      );
      if (preferredSigType !== 0 && preferredSigType !== 1) {
        await addCandidate(
          preferredSigType,
          proxyResolution.address,
          `${proxyResolution.source}_sigType${preferredSigType}`,
          proxyResolution.deployed,
        );
      }
    }
    ...
    if (!pureEoaPreferred && getConfiguredFunderAddress()) {
      try {
        ...
        if (derivedCheck?.ok && this._isAddress(derivedCheck.address)) {
          ...
          if (!alreadyCandidate) {
            ...
            await addCandidate(
              ...
              "auto_derived",
              derivedCheck.deployed,
            );
            if (preferredSigType !== 0 && preferredSigType !== 1) {
              await addCandidate(
                preferredSigType,
                derivedCheck.address,
                `auto_derived_sigType${preferredSigType}`,
                derivedCheck.deployed,
              );
            }
          }
    ...
    const probe = async (candidate) => {
      ...
      const updated = await withClobAuthContext(
        ...
        () =>
          withClobCallTimeout(
            () => candidate.client.updateBalanceAllowance({ asset_type: "COLLATERAL" }),
            12000,
            `CLOB_UPDATE_BALANCE_TIMEOUT_sig${candidate.signatureType}`,
          ).catch((e) => ({ error: e?.message || String(e), status: e?.response?.status || null })),
      );
      if (
        updated &&
      ...
      const balanceAllowance = await withClobAuthContext(
        ...
        () =>
          withClobCallTimeout(
            () => candidate.client.getBalanceAllowance({ asset_type: "COLLATERAL" }),
            12000,
            `CLOB_GET_BALANCE_TIMEOUT_sig${candidate.signatureType}`,
          ).catch((e) => ({ error: e?.message || String(e), status: e?.response?.status || null })),
      );
      if (
        balanceAllowance &&
  ...
  _selectTradeProbe(probes, preferredSigType, closedOnly) {
    ...
    const preferred = this._rankTradeProbes(
      probes.filter((probe) => probe.signatureType === preferredSigType),
      preferredSigType,
    )[0] || null;
    const ready = this._rankTradeProbes(
      probes.filter((probe) => !closedOnly && !!probe.ok),
      preferredSigType,
    );
  ...
  _rankTradeProbes(probes, preferredSigType) {
    const score = (probe) => {
      let out = 100;
      if (probe.signatureType === preferredSigType) out -= 40;
      const source = String(probe.source || "").toLowerCase();
      if (source.includes("auto_derived_sigtype")) out -= 30;
      else if (source.includes("proxy_derived")) out -= 20;
      else if (source.includes("auto_derived")) out -= 10;
      else if (source.includes("env")) out += 10;
      if (probe.deployed === false) out += 20;
      return out;
    };
    return [...(probes || [])].sort((a, b) => score(a) - score(b));
  }
  ...
  _buildAttemptOrder(probes, preferredSigType, closedOnly) {
    ...
    const readyFunders = new Set(
      ready
        .map((probe) => String(probe.funderAddress || "").toLowerCase())
        .filter(Boolean),
    );
    const retryablePreferred = this._rankTradeProbes(
      probes.filter((probe) => {
        if (probe.ok || probe.signatureType !== preferredSigType) return false;
        const funder = String(probe.funderAddress || "").toLowerCase();
        if (!funder || !readyFunders.has(funder)) return false;
        return this._isRetryableAuthError(
          `${probe.error || ""} ${probe.refreshError || ""}`,
        );
      }),
      preferredSigType,
    );
    ...
    for (const probe of retryablePreferred) {
  ...
  _isRetryableAuthError(message) {
    ...
    return (
      ...
      (raw.includes("order signer") && raw.includes("api key")) ||
      (raw.includes("signer address") && raw.includes("api key")) ||
  ...
  _buildRawOrderAmounts(side, size, price) {
    ...
    if (String(side).toUpperCase() === "BUY") {
      ...
      if (decimalPlaces(rawMakerAmt) > 4) {
        ...
        if (decimalPlaces(rawMakerAmt) > 4) rawMakerAmt = roundDown(rawMakerAmt, 4);
      }
    ...
    if (decimalPlaces(rawTakerAmt) > 4) {
      ...
      if (decimalPlaces(rawTakerAmt) > 4) rawTakerAmt = roundDown(rawTakerAmt, 4);
    }
    return { side: "SELL", rawMakerAmt, rawTakerAmt };
  }

  async _buildPoly1271SignedOrder(candidate, tokenId, price, shares, side) {
    if (!ethers || !this.wallet) {
      throw new Error("POLY_1271 raw signing requires ethers wallet");
    }
    const funder = String(candidate?.funderAddress || "").trim();
    if (!ethers.utils.isAddress(funder)) {
      throw new Error(`Invalid POLY_1271 funder address: ${funder || "missing"}`);
    }
    const { rawMakerAmt, rawTakerAmt, side: normalizedSide } = this._buildRawOrderAmounts(side, shares, price);
    const order = {
      salt: generateClobSafeSalt(),
      // Polymarket V2 deposit-wallet flow:
      // - `maker` is the deposit wallet (funder) that holds collateral
      // - `signer` must match the API key address (EOA) used to authenticate
      maker: ethers.utils.getAddress(funder),
      signer: ethers.utils.getAddress(this.wallet.address),
      tokenId: String(tokenId),
      makerAmount: ethers.utils.parseUnits(String(rawMakerAmt), USDC_DECIMALS).toString(),
      takerAmount: ethers.utils.parseUnits(String(rawTakerAmt), USDC_DECIMALS).toString(),
      side: normalizedSide,
      signatureType: 3,
    ...
    const orderValue = {
      ...order,
      side: normalizedSide === "BUY" ? 0 : 1,
    };
    const contentsHash = ethers.utils._TypedDataEncoder.hashStruct("Order", orderTypes, orderValue);
    const appDomainSeparator = ethers.utils._TypedDataEncoder.hashDomain(appDomain);
    const walletDomain = {
      name: "DepositWallet",
      version: "1",
      chainId: POLY_CHAIN_ID,
      verifyingContract: ethers.utils.getAddress(funder),
      salt: BYTES32_ZERO,
    };
    const wrapperTypes = {
      TypedDataSign: [
    ...
    const innerSignature = await this.wallet._signTypedData(walletDomain, wrapperTypes, {
      contents: contentsHash,
      name: appDomain.name,
      version: appDomain.version,
      chainId: appDomain.chainId,
      verifyingContract: appDomain.verifyingContract,
      salt: BYTES32_ZERO,
    });
    ...
    order.signature = `0x${normalizeHexNoPrefix(innerSignature)}${normalizeHexNoPrefix(appDomainSeparator)}${normalizeHexNoPrefix(contentsHash)}${Buffer.from(contentsType, "utf8").toString("hex")}${uint16Hex(contentsType.length)}`;
    return order;
  }
  ...
  async _placeOrderWithCandidate(candidate, tokenId, price, shares, side, opts = {}) {
    const placeOnce = async () => {
      ...
      let response = null;
      if (CONFIG.POLYMARKET_RAW_POLY1271_SIGNING && candidate.signatureType === 3 && typeof candidate.client.postOrder === "function") {
        const order = await this._buildPoly1271SignedOrder(candidate, tokenId, price, shares, side);
        response = await candidate.client.postOrder(order, orderType);
      } else if (typeof candidate.client.createAndPostOrder === "function") {
      ...
      return { response, routeBefore, routeAfter: getClobRoutingDiagnostics(), orderOptions, orderType };
    };
    ...
    try {
      const result = await withClobAuthContext(
        ...
        candidate.funderAddress,
        async () => {
          let { response, routeBefore, routeAfter, orderOptions, orderType } = await placeOnce();
          const firstFailure = response && !response.orderID ? buildClobFailureDiagnostic(response) : null;
          const firstSummary = formatClobFailureDiagnostic(firstFailure);
          if (firstSummary && this._isRetryableAuthError(firstSummary)) {
            const refreshed = await this._refreshOrderSignerCredsForCandidate(
              candidate,
              "order-signer-api-key-mismatch",
            );
            if (refreshed.ok) {
              console.log(
                ` Retrying CLOB order after signer/API-key credential refresh (${refreshed.source})`,
              );
              ({ response, routeBefore, routeAfter, orderOptions, orderType } = await placeOnce());
            }
          }
            ...
            for (let attempt = 1; attempt <= 3; attempt++) {
              ...
              try {
                ...
                if (orderStatus) {
                  ...
                  const normalizedMatchedShares = normalizeClobMatchedShares(
                    orderStatus.size_matched,
                    shares,
                  );
                  matchedShares = normalizedMatchedShares.matchedShares;
                  const normalizationWarning = formatMatchedSharesNormalization(
                    normalizedMatchedShares,
                  );
                  if (normalizationWarning) {
                    console.warn(`⚠️ CLOB size_matched normalized: ${normalizationWarning}`);
                  }
  ...
  async getTradeDiagnostics(opts = {}) {
    const ttlMs = opts.ttlMs || 30000;
    const probeState = await this._probeTradeClients({ ttlMs });
    const probes = Array.isArray(probeState?.probes) ? probeState.probes : [];
    return {
      ok: !!probeState?.ok,
      reason: probeState?.reason || null,
      walletAddress: this.wallet?.address || null,
      configuredFunderAddress: getConfiguredFunderAddress() || null,
      preferredSigType: probeState?.preferredSigType ?? null,
      forceDeriveCreds: !!CONFIG.POLYMARKET_FORCE_DERIVE_CREDS,
      autoDeriveCreds: !!CONFIG.POLYMARKET_AUTO_DERIVE_CREDS,
      hasStaticCreds: !!(
        CONFIG.POLYMARKET_API_KEY &&
        CONFIG.POLYMARKET_SECRET &&
        CONFIG.POLYMARKET_PASSPHRASE
      ),
      proxyResolution: probeState?.proxyResolution || null,
      closedOnly: probeState?.closedOnly ?? null,
      closedOnlyErr: probeState?.closedOnlyErr || null,
      probes: probes.map((probe) => ({
        signatureType: probe.signatureType,
        funderAddress: probe.funderAddress,
        source: probe.source || null,
        deployed: probe.deployed ?? null,
        ok: !!probe.ok,
        balanceRaw: probe.balanceRaw ?? null,
        balance: probe.balanceRaw != null ? parseFloat(String(probe.balanceRaw)) / 1e6 : null,
        allowanceMaxRaw: probe.allowanceMaxRaw ?? null,
        allowanceMaxSpender: probe.allowanceMaxSpender || null,
        refreshed: probe.refreshed ?? null,
        refreshError: probe.refreshError || null,
  ...
  async placeOrder(tokenId, price, shares, side = "BUY") {
    ...
    const errors = [];
    const orderAttemptTimeoutMs = Math.max(
      10000,
      Number(process.env.CLOB_ORDER_ATTEMPT_TIMEOUT_MS || 45000),
    );
    for (let index = 0; index < attemptOrder.length; index++) {
      ...
      for (let geoRetry = 0; geoRetry <= maxGeoblockRetries; geoRetry++) {
        ...
        console.log(
          ` CLOB order attempt ${index + 1}/${attemptOrder.length}: sigType=${candidate.signatureType} funder=${candidate.funderAddress || "none"} ${side} ${shares} @ ${price}`,
        );
        result = await withClobCallTimeout(
          () =>
            this._placeOrderWithCandidate(
              candidate,
              tokenId,
              price,
              shares,
          ...
          orderAttemptTimeoutMs,
          `CLOB_ORDER_ATTEMPT_TIMEOUT_sig${candidate.signatureType}`,
        ).catch((e) => ({
          success: false,
          error: `Order attempt timed out or failed before response: ${e.message || String(e)}`,
          signatureType: candidate.signatureType,
          funderAddress: candidate.funderAddress,
        }));
  ...
  async getClobCollateralBalance(force = false) {
    ...
    try {
      ...
      await withClobAuthContext(sel.sigType, authFunderAddress, () =>
        withClobCallTimeout(
          () => sel.client.updateBalanceAllowance({ asset_type: "COLLATERAL" }),
          12000,
          `CLOB_UPDATE_BALANCE_TIMEOUT_sig${sel.sigType}`,
        ).catch(() => {}),
      );
      const ba = await withClobAuthContext(sel.sigType, authFunderAddress, () =>
        withClobCallTimeout(
          () => sel.client.getBalanceAllowance({ asset_type: "COLLATERAL" }),
          12000,
          `CLOB_GET_BALANCE_TIMEOUT_sig${sel.sigType}`,
        ).catch((e) => ({ error: e.message || String(e), status: e?.response?.status || null })),
      );
      const baError =
        ba &&
