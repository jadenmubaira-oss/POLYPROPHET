    ...
    async ensureCreds() {
        ...
        this._deriveCredsPromise = (async () => {
            try {
                ...
                // Try sigType=0 first (standard L1 auth — the derive endpoint often requires this)
                // then fall back to sigType=1 if configured
                const sigTypes = [0];
                const configuredSig = Number(CONFIG.POLYMARKET_SIGNATURE_TYPE) || 0;
                if (configuredSig === 1) sigTypes.push(1);

                let creds = null;
                for (const st of sigTypes) {
                    try {
                        const funderRaw = String(CONFIG.POLYMARKET_ADDRESS || '').trim();
                        const funder = st === 1 && this._isAddress(funderRaw) ? funderRaw : (this.wallet.address || undefined);
                        console.log(`🔐 Trying derive with sigType=${st} funder=${String(funder).slice(0,10)}...`);
                        const tmp = new ClobClient(host, POLY_CHAIN_ID, this.wallet, undefined, st, funder);
                        ...
                        const result = await Promise.race([derivePromise, timeoutPromise]);

                        console.log(`🔐 sigType=${st} response: ${JSON.stringify(result).slice(0, 300)}`);

                        if (result && (result.key || result.apiKey) && (result.secret || result.apiSecret)) {
                            creds = result;
                            break;
                        }
                    } catch (e) {
                        console.log(`🔐 sigType=${st} failed: ${e.message}`);
                    }
                }
