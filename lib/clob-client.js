...
try {
    ({ CallType, encodeProxyTransactionData } = require('@polymarket/builder-relayer-client'));
} catch (e) {
    console.log('⚠️ @polymarket/builder-relayer-client not installed — proxy auto-redemption disabled');
}

try {
    ({ deriveProxyWallet } = require('@polymarket/builder-relayer-client/dist/builder'));
} catch (e) {
    console.log('⚠️ builder-relayer proxy helpers unavailable — proxy auto-redemption disabled');
}

try {
    ({ getContractConfig: getRelayContractConfig } = require('@polymarket/builder-relayer-client/dist/config'));
} catch (e) {
    console.log('⚠️ builder-relayer config helpers unavailable — proxy auto-redemption disabled');
}

try {
    ({ BuilderConfig } = require('@polymarket/builder-signing-sdk'));
} catch (e) {
    console.log('⚠️ @polymarket/builder-signing-sdk not installed — builder-auth relayer submits disabled');
}
...
class PolymarketCLOB {
        ...
        this._deriveCredsPromise = (async () => {
            try {
                ...
                const tryDerive = async (sigType, funder, label) => {
                    try {
                        console.log(` Deriving creds: sigType=${sigType} funder=${funder?.slice(0, 10)}... (${label})`);
                        const tmp = new ClobClient(host, POLY_CHAIN_ID, this.wallet, undefined, sigType, funder);
                        const raw = await Promise.race([
                            tmp.deriveApiKey(),
                            new Promise((_, rej) => setTimeout(() => rej(new Error('DERIVE_TIMEOUT_20s')), 20000))
                        ]);
                ...
                if (prefSigType === 1 && proxyAddr && !attempts.some(a => a[0] === 1)) {
                ...
                for (const [sigType, funder, label] of attempts) {
                    ...
                    if (creds) {
                        ...
                        console.log(` API creds derived via ${label} (key=${creds.key.slice(0, 8)}...)`);
                        return { ok: true, source: `derived-${label}` };
                    }
                }

                return { ok: false, reason: 'All derive attempts returned empty creds' };
            } catch (e) {
    ...
    _getRelayerAuthMode() {
        if (String(CONFIG.POLYMARKET_RELAYER_API_KEY || '').trim()) return 'relayer_api_key';
    ...
    async _ensureBuilderCreds() {
        ...
        if (this._deriveBuilderCredsPromise) return await this._deriveBuilderCredsPromise;
        if (!ClobClient || !this.wallet) return false;
        const hasClobCreds = !!(CONFIG.POLYMARKET_API_KEY && CONFIG.POLYMARKET_SECRET && CONFIG.POLYMARKET_PASSPHRASE);
        ...
        this._deriveBuilderCredsPromise = (async () => {
            try {
                ...
                const funder = sigType === 1 ? getConfiguredFunderAddress() : this.wallet.address;
                const creds = { key: CONFIG.POLYMARKET_API_KEY, secret: CONFIG.POLYMARKET_SECRET, passphrase: CONFIG.POLYMARKET_PASSPHRASE };
                const tmp = new ClobClient(host, POLY_CHAIN_ID, this.wallet, creds, sigType, funder);
                const raw = await Promise.race([
                    tmp.createBuilderApiKey(),
                    new Promise((_, rej) => setTimeout(() => rej(new Error('BUILDER_DERIVE_TIMEOUT_20s')), 20000))
                ]);
                const k = String(raw?.key || raw?.apiKey || '').trim();
                const s = String(raw?.secret || raw?.apiSecret || '').trim();
                const p = String(raw?.passphrase || raw?.apiPassphrase || '').trim();
                if (k && s && p) {
                    ...
                    console.log(` Builder API key auto-derived (key=${k.slice(0, 8)}...)`);
                    return true;
                }
    ...
    _getRelayerApiKeyAddress() {
        const configured = String(CONFIG.POLYMARKET_RELAYER_API_KEY_ADDRESS || '').trim();
        if (this._isAddress(configured)) return ethers.utils.getAddress(configured);
        return this._isAddress(this.wallet?.address) ? ethers.utils.getAddress(this.wallet.address) : null;
    }

    _getRelayerRequestConfig(extra = {}, useProxy = false) {
        const requestConfig = Object.assign({ timeout: 30000, proxy: false }, extra || {});
    ...
    _formatAxiosError(error) {
        if (axios?.isAxiosError?.(error)) {
            ...
            return {
                status,
                message: status ? `HTTP ${status}: ${detail || error.message}` : (detail || error.message),
                payload
            };
        }
        return {
    ...
    _isProxyTransportError(error) {
        ...
        return Number(formatted?.status) === 407
    ...
    async _buildRelayerSubmitHeaders(path, body) {
        ...
        if (authMode === 'relayer_api_key') {
            ...
            if (!apiKey || !ownerAddress) {
                return {
                    ok: false,
                    authMode,
                    error: 'PROXY_REDEEM_AUTH_INVALID (RELAYER_API_KEY or RELAYER_API_KEY_ADDRESS missing/invalid)'
                };
            }
            return {
                ok: true,
        ...
        if (authMode === 'builder') {
            if (!BuilderConfig) {
                return {
                    ok: false,
                    authMode,
                    error: 'PROXY_REDEEM_AUTH_INVALID (@polymarket/builder-signing-sdk unavailable)'
                };
            }
            ...
            try {
                const builderConfig = new BuilderConfig({
                    localBuilderCreds: {
                        ...
                        passphrase: String(CONFIG.POLYMARKET_BUILDER_PASSPHRASE || '').trim()
                    }
                });
                const headers = await builderConfig.generateBuilderHeaders('POST', path, body);
                if (!headers) {
                    return {
                        ok: false,
                        authMode,
                        error: 'PROXY_REDEEM_AUTH_INVALID (builder header generation returned empty payload)'
                    };
                }
        ...
        return {
            ok: false,
            authMode: null,
            error: 'PROXY_REDEEM_AUTH_MISSING (set POLYMARKET_RELAYER_API_KEY or POLYMARKET_BUILDER_* creds)'
        };
    }
    ...
    async _buildProxyRelayRequest(encodedData, relayPayload, metadata = '', transactionCount = 1) {
        if (!ethers) throw new Error('ethers not available');
        if (!getRelayContractConfig) throw new Error('builder-relayer config helpers unavailable');

        const signerAddress = this._isAddress(this.wallet?.address) ? ethers.utils.getAddress(this.wallet.address) : null;
        ...
        if (typeof deriveProxyWallet === 'function') {
            proxyWallet = deriveProxyWallet(signerAddress, proxyContracts.ProxyFactory);
        }
        ...
        if (!this._isAddress(proxyWallet)) {
            throw new Error('Unable to derive proxy wallet address for relayer submit');
        }
        ...
        const dataToHash = ethers.utils.hexConcat([
            ...
            to,
            encodedData,
            ethers.utils.hexZeroPad(ethers.BigNumber.from(relayerFee).toHexString(), 32),
            ethers.utils.hexZeroPad(ethers.BigNumber.from(gasPrice).toHexString(), 32),
            ethers.utils.hexZeroPad(ethers.BigNumber.from(gasLimit).toHexString(), 32),
            ethers.utils.hexZeroPad(ethers.BigNumber.from(String(relayPayload?.nonce || '0')).toHexString(), 32),
            proxyContracts.RelayHub,
            relayPayload.address
        ]);
        const relayHash = ethers.utils.keccak256(dataToHash);
        const signature = await this.wallet.signMessage(ethers.utils.arrayify(relayHash));
    ...
    async _pollRelayerTransaction(transactionID, maxPolls = 100, pollMs = 2000) {
        for (let attempt = 0; attempt < maxPolls; attempt++) {
            ...
            if (txn && ['STATE_MINED', 'STATE_CONFIRMED'].includes(String(txn.state || ''))) {
                return txn;
            }
            if (txn && ['STATE_FAILED', 'STATE_INVALID'].includes(String(txn.state || ''))) {
                return txn;
            }
    ...
    async _submitProxyRelayTransactions(transactions, metadata = '') {
        ...
        if (!axios || !CallType || !encodeProxyTransactionData || !getRelayContractConfig) {
            return {
                success: false,
                error: 'PROXY_REDEEM_UNAVAILABLE (builder relayer dependencies missing)',
                requiresManual: true
            };
        }
        ...
        try {
            ...
            if (!authMode) {
                return {
                    success: false,
                    error: 'PROXY_REDEEM_AUTH_MISSING (auto-derivation failed; set POLYMARKET_RELAYER_API_KEY or POLYMARKET_BUILDER_* creds)',
                    authMode: null,
                    requiresManual: true
                };
            }

            const proxyTransactions = (Array.isArray(transactions) ? transactions : [])
                .map((txn) => ({
                    to: txn?.to,
                ...
                .filter((txn) => this._isAddress(txn.to) && typeof txn.data === 'string' && txn.data.startsWith('0x'));

            if (proxyTransactions.length === 0) {
                return { success: false, error: 'No valid proxy transactions to submit' };
            }
            ...
            const request = await this._buildProxyRelayRequest(encodedData, relayPayload, metadata, proxyTransactions.length);
            const body = JSON.stringify(request);
            const headerState = await this._buildRelayerSubmitHeaders('/submit', body);
            if (!headerState?.ok) {
                return {
                    success: false,
            ...
            console.log(` Executing proxy relayer submit (${proxyTransactions.length} tx, auth=${headerState.authMode})`);
            ...
            const result = await this._pollRelayerTransaction(transactionID, 100, 2000);
            if (!result) {
                return {
                    success: false,
            ...
            if (['STATE_FAILED', 'STATE_INVALID'].includes(String(result.state || ''))) {
                return {
                    success: false,
            ...
            const authFailure = [401, 403].includes(Number(formatted.status))
                || /invalid authorization|not allowed/i.test(String(formatted.message || ''));
            return {
                success: false,
    ...
    async _probeTradeClients(opts = {}) {
        ...
        if (getConfiguredFunderAddress()) {
            try {
                const derivedCheck = await this._deriveProxyWalletAddress(Math.max(ttlMs, 60000));
                if (derivedCheck?.ok && this._isAddress(derivedCheck.address)) {
                    const alreadyCandidate = candidates.some(c =>
                        c.funderAddress?.toLowerCase() === derivedCheck.address.toLowerCase());
                    if (!alreadyCandidate) {
                        console.log(`⚠️ Auto-derived proxy ${derivedCheck.address} differs from configured POLYMARKET_ADDRESS ${getConfiguredFunderAddress()} — adding as extra candidate`);
                        ...
                        candidates.push({
                            ...
                            client: new ClobClient(host, POLY_CHAIN_ID, this.wallet, creds, 1, derivedCheck.address)
                        });
                    }
                }
            } catch (e) {
        ...
        const failedProxyProbes = probes.filter(p => p.signatureType === 1 && (p.status === 401 || !p.ok));
        for (const failedProbe of failedProxyProbes) {
            const proxyAddr = failedProbe.funderAddress;
            if (!this._isAddress(proxyAddr)) {
                console.log(` Proxy probe ${failedProbe.funderAddress?.slice(0,10)} ${failedProbe.status || 'failed'} — invalid funder address`);
                continue;
            }
            ...
            if (verifiedProxyBalance <= 0) {
                console.log(` Proxy probe ${proxyAddr.slice(0,10)} ${failedProbe.status || 'failed'} — no verified collateral balance at that address`);
                continue;
            }

            const balRaw = String(Math.round(verifiedProxyBalance * 1e6));
            console.log(` Proxy probe ${proxyAddr.slice(0,10)} ${failedProbe.status || 'failed'} — forcing trade-ready with same-address on-chain fallback $${verifiedProxyBalance.toFixed(2)}`);
            failedProbe.balanceRaw = balRaw;
            failedProbe.allowanceMaxRaw = '115792089237316195423570985008687907853269984665640564039457584007913129639935';
            failedProbe.ok = true;
    ...
    _isGeoblockError(message) {
        const raw = String(message || '').toLowerCase();
        return raw.includes('trading restricted') || raw.includes('geoblock') || raw.includes('status\":403');
    }
    ...
    async placeOrder(tokenId, price, shares, side = 'BUY') {
        ...
        for (let index = 0; index < attemptOrder.length; index++) {
            ...
            for (let geoRetry = 0; geoRetry <= maxGeoblockRetries; geoRetry++) {
                if (geoRetry > 0) {
                    console.log(`  Geoblock retry ${geoRetry}/${maxGeoblockRetries} after 3s...`);
                    await new Promise(r => setTimeout(r, 3000));
                }
                result = await this._placeOrderWithCandidate(candidate, tokenId, price, shares, side);
    ...
    getSelectedFunderAddress() {
        const candidate = this._tradeReadyCache?.selected?.funderAddress
            || getKnownProxyFunderAddress()
            || this.wallet?.address
            || null;
        return this._isAddress(candidate) ? ethers.utils.getAddress(candidate) : null;
    }
    ...
    async getTokenBalance(tokenId, addressOverride = null) {
        ...
        try {
            const targetAddress = addressOverride && this._isAddress(addressOverride)
    ...
    async getTokenBalanceAcrossHolders(tokenId, preferredAddress = null) {
        ...
        if (addresses.length === 0) {
            return { success: false, error: 'No holder addresses available', balance: 0, tokenId, address: null, balances: [] };
        }

        const balances = [];
        let best = null;
        for (const address of addresses) {
            const result = await this.getTokenBalance(tokenId, address).catch((e) => ({ success: false, error: e.message, balance: 0, tokenId, address }));
            const entry = {
            ...
            balances.push(entry);
            if (!best || (entry.success && entry.balance > best.balance) || (!best.success && entry.success)) {
                best = entry;
            }
        }

        const normalizedPreferred = this._isAddress(preferredAddress) ? ethers.utils.getAddress(preferredAddress) : null;
        ...
        const zeroVerified = preferredEntry
            ...
            : (balances.length > 0 && balances.every((entry) => entry.success && entry.balance <= 0));
        if (best?.success) {
            return {
                success: true,
        ...
        return {
            success: false,
            error: balances.find((entry) => !entry.success && entry.error)?.error || 'Token balance unavailable',
            balance: 0,
            tokenId,
    ...
    async redeemPosition(conditionId, holderAddress = null) {
        ...
        try {
            ...
            if (targetHolder && signerAddress && targetHolder !== signerAddress) {
                return await this._redeemPositionViaProxyRelayer(conditionId, targetHolder);
            }
            ...
            return { success: receipt?.status === 1, txHash: tx.hash, receipt, address: signerAddress };
        } catch (e) {
            return { success: false, error: e.message };
        }
    }
    ...
    getStatus() {
        ...
        return {
            ...
            relayerAuthMode: this._getRelayerAuthMode(),
            relayerAuthReady: !!this._getRelayerAuthMode(),
            relayerAuthConfigured: !!this._getRelayerAuthMode(),
            builderAutoDerivable: !!(ClobClient && this.wallet && CONFIG.POLYMARKET_API_KEY && CONFIG.POLYMARKET_SECRET && CONFIG.POLYMARKET_PASSPHRASE),
            proxyRedeemAuthReady: !!this._getRelayerAuthMode() || !!(ClobClient && this.wallet && CONFIG.POLYMARKET_API_KEY && CONFIG.POLYMARKET_SECRET && CONFIG.POLYMARKET_PASSPHRASE),
