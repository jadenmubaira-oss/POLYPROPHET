...
class PolymarketCLOB {
    ...
    async _probeTradeClients(opts = {}) {
        ...
        const primarySigType1Funder = configuredFunderAddress || signerAddress;
        ...
        if (this._isAddress(primarySigType1Funder)) {
            candidates.push({
                ...
                source: configuredFunderAddress ? 'env' : 'wallet',
    ...
    async _placeOrderWithCandidate(candidate, tokenId, price, shares, side) {
        try {
            const order = await withClobAuthContext(candidate.signatureType, candidate.funderAddress, () => candidate.client.createOrder({
                tokenID: tokenId,
                price: price,
                size: shares,
                side: side
            ...
            const response = await withClobAuthContext(candidate.signatureType, candidate.funderAddress, () => candidate.client.postOrder(order));
            ...
            if (response && response.orderID) {
                ...
                for (let attempt = 1; attempt <= 3; attempt++) {
                    ...
                    try {
                        const orderStatus = await withClobAuthContext(candidate.signatureType, candidate.funderAddress, () => candidate.client.getOrder(response.orderID));
                        if (orderStatus) {
                ...
                if (safeMatchedShares + eps < shares) {
                    try {
                        await withClobAuthContext(candidate.signatureType, candidate.funderAddress, () => candidate.client.cancelOrder({ orderID: response.orderID }));
                    } catch {}
                }
