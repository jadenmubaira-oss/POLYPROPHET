...
class PolymarketCLOB {
    ...
    async _probeTradeClients(opts = {}) {
        ...
        // If preferred sigType probe got 401, try re-deriving creds once
        const preferredProbe = probes.find(p => p.signatureType === preferredSigType);
        if (preferredProbe && preferredProbe.status === 401 && !this._hasRetriedDerive) {
            this._hasRetriedDerive = true;
            console.log(' Preferred probe got 401 — re-deriving API creds...');
            const rederived = await this.rederiveCreds();
            if (rederived?.ok) {
                const newCreds = this._buildCreds();
                // Rebuild candidates with new creds and re-probe
                const newProbes = [];
                for (const candidate of candidates) {
                    const newClient = new ClobClient('https://clob.polymarket.com', POLY_CHAIN_ID, this.wallet, newCreds, candidate.signatureType, candidate.funderAddress);
                    candidate.client = newClient;
                    newProbes.push(await probe(candidate));
                }
                return { ok: true, reason: null, probes: newProbes, preferredSigType, closedOnly, closedOnlyErr, proxyResolution };
            }
        }
