...
class PolymarketCLOB {
    ...
    async _probeTradeClients(opts = {}) {
        ...
        // If any sigType=1 probe got 401 or has no balance, check on-chain USDC at both EOA
        // and all proxy addresses to find the real funds.
        const configAddr = getConfiguredFunderAddress();
        const failedProxyProbes = probes.filter(p => p.signatureType === 1 && (p.status === 401 || !p.ok));
        if (failedProxyProbes.length > 0) {
            const eoaProbe = probes.find(p => p.signatureType === 0);
            let bestBalance = 0;
            let bestSource = '';
            if (eoaProbe?.balanceRaw) {
                const eoaBal = Number(eoaProbe.balanceRaw) / 1e6;
                if (eoaBal > bestBalance) { bestBalance = eoaBal; bestSource = 'eoa_clob'; }
            }
            try {
                const onChainEoa = await this.getOnChainUsdcBalance();
                if (onChainEoa?.success && onChainEoa.balance > bestBalance) { bestBalance = onChainEoa.balance; bestSource = 'eoa_onchain'; }
            } catch {}
            // Check on-chain USDC at each proxy address
            const proxyAddresses = [...new Set(probes.filter(p => p.signatureType === 1).map(p => p.funderAddress).filter(Boolean))];
            for (const proxyAddr of proxyAddresses) {
                try {
                    const onChainProxy = await this.getOnChainUsdcBalance(proxyAddr);
                    if (onChainProxy?.success && onChainProxy.balance > bestBalance) { bestBalance = onChainProxy.balance; bestSource = `proxy_onchain(${proxyAddr.slice(0,10)})`; }
                } catch {}
            }

            for (const failedProbe of failedProxyProbes) {
                if (bestBalance <= 0) {
                    console.log(` Proxy probe ${failedProbe.funderAddress?.slice(0,10)} ${failedProbe.status || 'failed'} — no verified collateral balance`);
                    continue;
                }
                const balRaw = String(Math.round(bestBalance * 1e6));
                console.log(` Proxy probe ${failedProbe.funderAddress?.slice(0,10)} ${failedProbe.status || 'failed'} — forcing trade-ready with $${bestBalance.toFixed(2)} fallback (${bestSource})`);
