...
class PolymarketCLOB {
  ...
  async ensureCreds() {
    ...
    if (hasCreds && !CONFIG.POLYMARKET_FORCE_DERIVE_CREDS)
  ...
  _buildAttemptOrder(probes, preferredSigType, closedOnly) {
    ...
    // For Polymarket V2 deposit-wallet flow, sigType=3 (POLY_1271) is the intended path.
    // Trying other signature types can produce confusing hard failures like:
    // "maker address not allowed, please use the deposit wallet flow".
    // So when the preferred sigType is 3 and we have any ready 3-probes, try ONLY those.
    if (preferredSigType === 3) {
      const preferredOnly = ready.filter((probe) => probe.signatureType === 3);
