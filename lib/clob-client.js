...
class PolymarketCLOB {
    ...
    async _probeTradeClients(opts = {}) {
        ...
        if (configAddr) {
            ...
            if (configProbe && configProbe.status === 401 && eoaProbe) {
                const eoaBalance = eoaProbe.balanceRaw ? BigInt(eoaProbe.balanceRaw) : 0n;
                // Check on-chain balance as fallback
                const onChainResult = await this.getOnChainUsdcBalance().catch(() => ({ balance: 0 }));
                const onChainBalance = Number(onChainResult?.balance || 0);
                const clobBalance = Number(eoaBalance) / 1e6;
                const bestBalance = Math.max(clobBalance, onChainBalance);
                if (bestBalance > 0) {
                    const balRaw = String(Math.round(bestBalance * 1e6));
                    console.log(` Proxy probe 401 but found $${bestBalance.toFixed(2)} via fallback — forcing trade-ready`);
