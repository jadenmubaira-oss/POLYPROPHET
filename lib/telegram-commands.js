 ...
 *   /proxytest RAW        — test a raw curl/proxy route without placing trades
...
const flySecrets = require('./fly-secrets');
const { testProxyInput } = require('./proxy-tester');
...
function getControlKeyboard() {
    return {
        inline_keyboard: [
            ...
            [
                { text: '🏁 Redeem Status', callback_data: 'pp_redeem_status' },
                { text: '🔐 Derive Redeem Auth', callback_data: 'pp_derive_redeem_auth' }
            ],
            [
                { text: '♻️ Auto Redeem', callback_data: 'pp_auto_redeem' },
                { text: '🗝 Fly Secrets', callback_data: 'pp_fly_secrets' }
            ],
            [
            ...
            [
                { text: '🧹 Rotation Reset', callback_data: 'pp_rotation_reset' }
            ]
        ]
    };
}
...
function getRotationResetConfirmKeyboard() {
    return {
        inline_keyboard: [
            [
                { text: 'Confirm Rotation Reset', callback_data: 'pp_confirm_rotation_reset' },
                { text: 'Cancel', callback_data: 'pp_cancel' }
            ],
            [
                { text: '🏁 Redeem Status', callback_data: 'pp_redeem_status' },
...
function cmdHelp() {
    replyText(
        ...
        `/redeem_status — redemption/auth queue status\n` +
        `/derive_redeem_auth — derive proxy redeem auth, optionally persist to Fly\n` +
        `/auto_redeem — derive auth, redeem winning tokens, refresh balance\n` +
        `/fly_secrets — show Fly secret automation readiness\n` +
        `/pause — pause trading\n` +
        ...
        `/proxytest RAW_PROXY_OR_CURL — non-trading proxy/geoblock/order-preflight test\n` +
...
function summarizeRedemptionStatus(exec = deps.tradeExecutor?.getStatus?.()) {
    const summary = exec?.redemptionQueueSummary || {};
    const provisional = exec?.provisionalOutcomeSummary || {};
    const wallet = deps.tradeExecutor?.clob?.getStatus?.() || {};
    const readiness = {
        autoRedeemAuthReady: !!wallet.proxyRedeemAuthReady,
        proxyRedeemAuthDerivable: !!wallet.proxyRedeemAuthDerivable,
        relayerAuthMode: wallet.relayerAuthMode || null,
        relayerAuthConfigured: !!wallet.relayerAuthConfigured,
        lastBuilderDerive: wallet.lastBuilderDerive || null
    };
    return {
        summary,
        provisional,
        ...
        lines: [
            ...
            `Queue: total=<b>${Number(summary.total || 0)}</b> actionable=<b>${Number(summary.actionable || 0)}</b> authBlocked=<b>${Number(summary.authBlocked || 0)}</b> manualRequired=<b>${Number(summary.manualRequired || 0)}</b>`,
            `Provisional: wins=<b>${Number(provisional.provisionalWins || 0)}</b> losses=<b>${Number(provisional.provisionalLosses || 0)}</b> walletSettled=<b>${provisional.walletSettled ? 'YES' : 'NO'}</b>`,
            `Auth ready: <b>${readiness.autoRedeemAuthReady ? 'YES' : 'NO'}</b> | derivable: <b>${readiness.proxyRedeemAuthDerivable ? 'YES' : 'NO'}</b> | mode: <code>${escHtml(readiness.relayerAuthMode || 'none')}</code>`,
            readiness.lastBuilderDerive ? `Last derive: ok=${readiness.lastBuilderDerive.ok ? 'YES' : 'NO'} source=${escHtml(readiness.lastBuilderDerive.source || 'n/a')} err=${escHtml(readiness.lastBuilderDerive.error || 'none')}` : `Last derive: none`,
            `Fly secret automation: ${flySecrets.publicState().configured ? '✅ configured' : '⚠️ not configured'}`
        ]
    };
}

function summarizeRedemptionEvents(redemptions, limit = 3) {
    const events = Array.isArray(redemptions?.events) ? redemptions.events : [];
    return events
        .filter((event) => event && event.outcome && event.outcome !== 'REDEEMED')
        .slice(0, limit)
        .map((event, idx) => {
            const reason = String(event.reason || 'unknown')
                .replace(/0x[a-fA-F0-9]{40,}/g, '0x…')
                .replace(/(api[_-]?key|secret|passphrase|authorization|token)[=:]\s*[^,\s}]+/gi, '$1=[redacted]')
                .replace(/[A-Za-z0-9_-]{48,}/g, '[redacted]')
                .slice(0, 220);
            return `${idx + 1}. ${escHtml(event.outcome)}: ${escHtml(reason)}`;
        });
}

function buildBuilderSecretMap(clob) {
    if (!clob) return null;
    if (typeof clob.getLastDerivedBuilderSecrets === 'function') {
        const derived = clob.getLastDerivedBuilderSecrets();
        if (derived) return derived;
    }
    if (
        CONFIG.POLYMARKET_BUILDER_API_KEY &&
        CONFIG.POLYMARKET_BUILDER_SECRET &&
        CONFIG.POLYMARKET_BUILDER_PASSPHRASE
    ) {
        return {
            POLYMARKET_BUILDER_API_KEY: CONFIG.POLYMARKET_BUILDER_API_KEY,
            POLYMARKET_BUILDER_SECRET: CONFIG.POLYMARKET_BUILDER_SECRET,
            POLYMARKET_BUILDER_PASSPHRASE: CONFIG.POLYMARKET_BUILDER_PASSPHRASE
        };
    }
    return null;
}

function buildClobSecretMap(clob) {
    if (!clob) return null;
    if (typeof clob.getLastDerivedClobSecrets === 'function') {
        const derived = clob.getLastDerivedClobSecrets();
        if (derived) return derived;
    }
    return null;
}

async function persistPolymarketSecretsIfAvailable(auth) {
    ...
    const secrets = Object.assign(
        {},
        buildClobSecretMap(clob) || {},
        auth?.ok ? (buildBuilderSecretMap(clob) || {}) : {}
    );
    if (!Object.keys(secrets).length) {
        return { success: false, skipped: true, reason: 'NO_DERIVED_POLYMARKET_SECRETS', state: flySecrets.publicState() };
    }
    return await flySecrets.setSecrets(secrets);
}

async function cmdFlySecrets() {
    const state = flySecrets.publicState();
    replyText(
        `<b>FLY SECRET AUTOMATION</b>\n` +
        `Enabled: <b>${state.enabled ? 'YES' : 'NO'}</b>\n` +
        `Token present: <b>${state.tokenPresent ? 'YES' : 'NO'}</b>\n` +
        `App: <code>${escHtml(state.appName || 'none')}</code>\n` +
        `Configured: <b>${state.configured ? 'YES' : 'NO'}</b>\n\n` +
        `To allow button-based persistence, set Fly secrets <code>TELEGRAM_FLY_SECRETS_ENABLED=true</code> and <code>FLY_API_TOKEN</code> or <code>TELEGRAM_FLY_API_TOKEN</code>. The bot never sends secret values to Telegram.`,
        { replyMarkup: getControlKeyboard() }
    );
}

async function cmdRedeemStatus() {
    try {
        const breakdown = await getFreshBalanceBreakdown(true);
        const status = summarizeRedemptionStatus();
        const lines = [
            ...status.lines,
            `Cash: <b>${fmtUsd(breakdown.tradingBalanceUsdc)}</b> | source=${escHtml(breakdown.source || 'unknown')}`
        ];
...
async function cmdDeriveRedeemAuth() {
    try {
        const clob = getWalletClient();
        if (!clob || typeof clob.ensureProxyRedeemAuth !== 'function') {
            return replyText('Redeem auth derivation unavailable on this deploy.', { replyMarkup: getControlKeyboard() });
        }
        replyText('🔐 Deriving proxy redeem auth now. No secret values will be sent to Telegram...');
        const auth = await clob.ensureProxyRedeemAuth();
        const fly = await persistPolymarketSecretsIfAvailable(auth);
        const status = summarizeRedemptionStatus();
        const flyLine = fly.success
            ? `Fly persistence: ✅ set ${fly.keys.join(', ')} release=${escHtml(fly.release?.version || fly.release?.id || 'created')}`
            : `Fly persistence: ⚠️ ${escHtml(fly.reason || fly.rawError || (fly.errors || []).join('; ') || 'not applied')}`;
        replyText(
            `<b>REDEEM AUTH DERIVE ${auth.ok ? '✅' : '❌'}</b>\n` +
            `Ready: <b>${auth.proxyRedeemAuthReady ? 'YES' : 'NO'}</b> | Derived: <b>${auth.derived ? 'YES' : 'NO'}</b> | Mode: <code>${escHtml(auth.authMode || 'none')}</code>\n` +
            `Last: ${escHtml(auth.lastBuilderDerive?.error || auth.lastBuilderDerive?.source || 'none')}\n` +
            `${flyLine}\n\n` +
            status.lines.slice(1).join('\n'),
            { replyMarkup: getControlKeyboard() }
        );
    } catch (e) {
        replyText(`🔐 <b>REDEEM AUTH DERIVE FAILED</b>\n${escHtml(e.message)}`, { replyMarkup: getControlKeyboard() });
    }
}

async function cmdAutoRedeem() {
    try {
        const exec = deps.tradeExecutor;
        if (!exec || typeof exec.checkAndRedeemPositions !== 'function') {
            return replyText('Auto-redeem unavailable on this deploy.', { replyMarkup: getControlKeyboard() });
        }
        replyText('♻️ Running auto-redeem: derive auth → redeem queue → refresh balance...');
        let auth = null;
        if (exec.clob?.ensureProxyRedeemAuth) auth = await exec.clob.ensureProxyRedeemAuth();
        const fly = await persistPolymarketSecretsIfAvailable(auth);
        const redemptions = await exec.checkAndRedeemPositions();
        const balance = typeof exec.refreshLiveBalance === 'function'
            ? await exec.refreshLiveBalance(true).catch((e) => ({ success: false, error: e.message }))
            : null;
        await deps.saveRuntimeState?.();
        const after = exec.getStatus?.() || {};
        const summary = after.redemptionQueueSummary || {};
        const success = !!redemptions?.success && Number(summary.actionable || 0) === 0;
        const eventLines = summarizeRedemptionEvents(redemptions);
        replyText(
            `<b>AUTO REDEEM ${success ? '✅ CLEAR' : '⚠️ NOT CLEAR'}</b>\n` +
            `Auth: ${auth?.ok ? '✅' : '❌'} mode=<code>${escHtml(auth?.authMode || 'none')}</code> err=${escHtml(auth?.lastBuilderDerive?.error || 'none')}\n` +
            `Redeemed: <b>${Number(redemptions?.redeemed || 0)}</b> failed=${Number(redemptions?.failed || 0)} skipped=${Number(redemptions?.skipped || 0)} remaining=<b>${Number(redemptions?.remaining || summary.total || 0)}</b>\n` +
            `After queue: actionable=<b>${Number(summary.actionable || 0)}</b> authBlocked=<b>${Number(summary.authBlocked || 0)}</b>\n` +
            `Balance: <b>${fmtUsd(balance?.tradingBalanceUsdc ?? after.balanceBreakdown?.tradingBalanceUsdc)}</b>\n` +
            (eventLines.length ? `Last errors:\n${eventLines.join('\n')}\n` : '') +
            `Fly persistence: ${fly.success ? '✅ applied' : `⚠️ ${escHtml(fly.reason || fly.rawError || (fly.errors || []).join('; ') || 'not applied')}`}`,
            { replyMarkup: getControlKeyboard() }
        );
    } catch (e) {
        replyText(`♻️ <b>AUTO REDEEM FAILED</b>\n${escHtml(e.message)}`, { replyMarkup: getControlKeyboard() });
    }
}

function conciseProxyTestLine(label, result) {
    const data = result?.data || {};
    const chunks = [`${label}: ${result?.ok ? '✅' : '❌'} status=${result?.status ?? 'n/a'}`];
    if (data.blocked != null) chunks.push(`blocked=${data.blocked}`);
    if (data.country) chunks.push(`country=${data.country}`);
    if (data.region) chunks.push(`region=${data.region}`);
    if (data.ip) chunks.push(`ip=${data.ip}`);
    if (data.org) chunks.push(`org=${data.org}`);
    if (result?.error) chunks.push(`error=${result.error}`);
    const body = typeof result?.body === 'string' && result.body
        ? result.body.slice(0, 160).replace(/\s+/g, ' ')
        : (typeof result?.data === 'string' ? result.data.slice(0, 160).replace(/\s+/g, ' ') : '');
    if (body) chunks.push(`body=${body}`);
    return chunks.join(' | ');
}

async function cmdProxyTest(argStr) {
    const input = String(argStr || '').trim();
    if (!input) {
        return replyText(
            `<b>PROXY TEST</b>\n` +
            `Usage:\n` +
            `<code>/proxytest curl -i --proxy HOST:PORT --proxy-user USER:PASS -k</code>\n\n` +
            `Or:\n` +
            `<code>/proxytest http://USER:PASS@HOST:PORT</code>\n\n` +
            `This performs non-trading checks only: public geoblock, CLOB /time, and invalid-payload CLOB /order preflight.`,
            { replyMarkup: getControlKeyboard() }
        );
    }
    replyText('🧪 Running non-trading proxy acceptance test. This may take up to ~2 minutes...');
    try {
        const result = await testProxyInput(input, { timeoutMs: 35000 });
        const pass = !!result.verdict?.pass;
        const lines = [
            `<b>PROXY TEST ${pass ? '✅ PASS / CANDIDATE' : '❌ FAIL / NO-GO'}</b>`,
            `Grade: <code>${escHtml(result.verdict.grade)}</code>`,
            `Route: <code>${escHtml(result.redactedProxyUrl)}</code>`,
            escHtml(result.verdict.summary),
            '',
            conciseProxyTestLine('BrightData', result.tests.brightDataWelcome),
            conciseProxyTestLine('ipinfo', result.tests.ipinfo),
            conciseProxyTestLine('PM geoblock', result.tests.publicGeoblock),
            conciseProxyTestLine('CLOB /time', result.tests.clobTime),
            conciseProxyTestLine('CLOB /order', result.tests.clobOrderPreflight),
            ''
        ];
        if (Array.isArray(result.verdict.warnings) && result.verdict.warnings.length) {
            lines.push('Warnings:');
            for (const warning of result.verdict.warnings.slice(0, 4)) lines.push(`- ${escHtml(warning)}`);
            lines.push('');
        }
        if (pass) {
            lines.push('Render envs to set:');
            lines.push(`<code>PROXY_URL=${escHtml(result.renderEnv.PROXY_URL)}</code>`);
            lines.push(`<code>CLOB_FORCE_PROXY=1</code>`);
            lines.push('');
            lines.push(`Next proof: ${escHtml(result.verdict.expectedNextProof)}`);
        } else {
            lines.push('Do not use this proxy for live trading. No Render env recommended.');
        }
        replyText(lines.join('\n'), { replyMarkup: getControlKeyboard() });
    } catch (e) {
        replyText(`🧪 PROXY TEST FAILED\n<code>${escHtml(e.message)}</code>`, { replyMarkup: getControlKeyboard() });
    }
}
...
function proofBalanceUsd(snapshot) {
    return snapshot?.balanceBreakdown?.tradingBalanceUsdc ?? snapshot?.tradingBalanceUsdc ?? null;
}
...
async function cmdLiveProof(argStr = '') {
    try {
        ...
        const lines = [
            ...
            proof.requestedExposure ? `Requested exposure cap: ${escHtml(proof.requestedExposure.shares)} shares @ ${escHtml(proof.requestedExposure.price)} = ~$${Number(proof.requestedExposure.maxNotionalUsd || 0).toFixed(4)}` : null,
            ...
            order.clobRoute?.lastRequest ? `CLOB route: ${escHtml(order.clobRoute.lastRequest.method)} ${escHtml(order.clobRoute.lastRequest.path)} via ${escHtml(order.clobRoute.lastRequest.agent)} (proxied=${escHtml(order.clobRoute.lastRequest.proxied)})` : null,
            `Balance before/after: ${fmtUsd(proofBalanceUsd(proof.beforeBalance))} → ${fmtUsd(proofBalanceUsd(proof.afterBalance))}`,
            escHtml(proof.note || '')
        ].filter(Boolean);
...
async function rotationResetFromTelegram() {
    const exec = deps.tradeExecutor;
    if (!exec || typeof exec.resetForAccountRotation !== 'function') {
        throw new Error('Rotation reset unavailable on this deploy.');
    }
    deps.riskManager.tradingPaused = true;
    const reset = exec.resetForAccountRotation({ preserveClosedPositions: true });
    const balance = typeof exec.refreshLiveBalance === 'function'
        ? await exec.refreshLiveBalance(true).catch((e) => ({ success: false, error: e.message }))
        : null;
    const balanceUsd = Number(balance?.tradingBalanceUsdc);
    if (Number.isFinite(balanceUsd) && typeof exec.resetMonitoringBaseline === 'function') {
        exec.resetMonitoringBaseline(balanceUsd, {
            source: 'telegram_rotation_reset_live_balance',
            markInternalEvent: false
        });
        deps.riskManager.bankroll = balanceUsd;
    }
    await deps.saveRuntimeState?.();
...
async function handleMessage(message) {
    ...
    switch (cmd) {
        ...
        case '/redeem_status': return cmdRedeemStatus();
        case '/derive_redeem_auth': return cmdDeriveRedeemAuth();
        case '/auto_redeem': return cmdAutoRedeem();
        case '/fly_secrets': return cmdFlySecrets();
        case '/rotation_reset':
            return replyText(
                '⚠️ <b>Confirm account rotation reset?</b>\nOnly use after old-account funds are moved/redeemed and new wallet/Fly secrets are installed. This clears bot-local open/pending/recovery/redemption queues and keeps trading paused.',
                { replyMarkup: getRotationResetConfirmKeyboard() }
            );
        case '/liveproof': return cmdLiveProof(argStr);
        case '/proxytest': return cmdProxyTest(argStr);
...
async function handleCallbackQuery(callbackQuery) {
    ...
    try {
        ...
        if (data === 'pp_redeem_status') {
            await cmdRedeemStatus();
            answerCallbackQuery(callbackId, 'Redeem status sent');
            return;
        }
        if (data === 'pp_derive_redeem_auth') {
            answerCallbackQuery(callbackId, 'Deriving redeem auth');
            await cmdDeriveRedeemAuth();
            return;
        }
        if (data === 'pp_auto_redeem') {
            answerCallbackQuery(callbackId, 'Auto-redeem started');
            await cmdAutoRedeem();
            return;
        }
        if (data === 'pp_fly_secrets') {
            await cmdFlySecrets();
            answerCallbackQuery(callbackId, 'Fly secrets status sent');
            return;
        }
        if (data === 'pp_withdraw') {
            await cmdWithdraw('');
        ...
        if (data === 'pp_rotation_reset') {
            replyText(
                '⚠️ <b>Confirm account rotation reset?</b>\nOnly use after old-account funds are moved/redeemed and new wallet/Fly secrets are installed. This clears bot-local open/pending/recovery/redemption queues and keeps trading paused.',
                { replyMarkup: getRotationResetConfirmKeyboard() }
            );
            answerCallbackQuery(callbackId, 'Confirm rotation reset');
            return;
        }
        ...
        if (data === 'pp_confirm_rotation_reset') {
            const result = await rotationResetFromTelegram();
            const after = result.reset?.after || {};
            replyText(
                `🧹 <b>ROTATION RESET COMPLETE</b>\n` +
                `Queues: redemption=<b>${Number(after.redemptionQueue || 0)}</b> recovery=<b>${Number(after.recoveryQueue || 0)}</b> pendingBuys=<b>${Number(after.pendingBuys || 0)}</b>\n` +
                `Balance: <b>${fmtUsd(result.balance?.tradingBalanceUsdc)}</b> source=${escHtml(result.balance?.source || 'unknown')}\n` +
                `Trading remains paused. Re-check Redeem Status and Dashboard before resume.`,
                { replyMarkup: getControlKeyboard() }
            );
            answerCallbackQuery(callbackId, 'Rotation reset complete');
            return;
        }
