...
async function persistBuilderSecretsIfAvailable(auth) {
    const clob = getWalletClient();
    const secrets = auth?.ok ? buildBuilderSecretMap(clob) : null;
    if (!secrets) {
        return { success: false, skipped: true, reason: 'NO_DERIVED_BUILDER_SECRETS', state: flySecrets.publicState() };
    }
...
async function cmdDeriveRedeemAuth() {
    try {
        ...
        const fly = await persistBuilderSecretsIfAvailable(auth);
        const status = summarizeRedemptionStatus();
