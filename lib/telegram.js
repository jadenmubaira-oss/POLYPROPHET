...
async function drain() {
    ...
    try {
        while (queue.length > 0) {
            ...
            await httpSend(next.text, cfg.chatId, cfg.botToken);
            markSent();
        }
    } finally {
        draining = false;
...
function sendMessage(text, priority = PRIORITY.MEDIUM, options = {}) {
    ...
    if (!shouldSend(priority)) return false;
    if (!options.allowDuplicate && isDuplicate(text)) return false;
...
function notifyTradeClose(result) {
    ...
    return sendMessage(msg, won ? PRIORITY.MEDIUM : PRIORITY.HIGH);
}
