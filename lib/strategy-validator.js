...
function resetState(options = {}) {
    ...
    state.lastPeriodicAt = 0;
    state.lastTradeCount = Number.isFinite(Number(options.tradeCount)) ? Number(options.tradeCount) : 0;
    state.strategyFileMtime = 0;
    state.strategyFilePath = '';
    state.firedSeverities = {};
    state.cooldownPreviousState = false;
    state.haltPreviousState = { error: false, tradeFailure: false };
    state.lastReport = options.clearReport === false ? state.lastReport : null;
    return {
        lastTradeCount: state.lastTradeCount,
        clearedReport: options.clearReport !== false
    };
}
