...
function evaluatePhase3Signal(strategy, market, currentMinute, currentHour) {
    const direction = String(strategy.direction).toUpperCase();
    const bandLo = Number(strategy.bandLo || strategy.priceMin || 0);
    const bandHi = Number(strategy.bandHi || strategy.priceMax || 1);
    const entryMin = Number(strategy.entryMinute);

    if (entryMin !== currentMinute) return null;
    if (!strategySupportsAsset(strategy, market.asset)) return null;

    if (strategy.type === 'hour_structural') {
        const reqHour = Number(strategy.hour);
        if (reqHour !== currentHour) return null;
        const entryPrice = direction === 'UP' ? market.yesPrice : market.noPrice;
        if (!entryPrice || entryPrice <= 0 || entryPrice >= 1) return null;
        if (entryPrice < bandLo || entryPrice > bandHi) return null;
        return { direction, entryPrice, signalType: 'hour_structural' };
    }

    if (strategy.type === 'per_asset_momentum') {
        const yPrices = market.minutePricesYes || {};
        const p0 = yPrices[0]?.last || yPrices[0];
        const p3 = yPrices[3]?.last || yPrices[3];
        if (!p0 || !p3 || p0 <= 0) return null;
        const mom = (p3 - p0) / p0;
        const momThresh = Number(strategy.momThresh);
        if (direction === 'UP' && mom <= momThresh) return null;
        if (direction === 'DOWN' && mom >= -momThresh) return null;
        const entryPrice = direction === 'UP' ? market.yesPrice : market.noPrice;
        if (!entryPrice || entryPrice <= 0 || entryPrice >= 1) return null;
        if (entryPrice < bandLo || entryPrice > bandHi) return null;
        return { direction, entryPrice, momentum: mom, signalType: 'per_asset_momentum' };
    }

    return null;
}

function evaluateMomentumSignal(strategy, market) {
    if (strategy.type !== 'momentum_follow') return null;
    const momThresh = Number(strategy.momThresh);
    const entryMinute = Number(strategy.entryMinute);
    const bandLo = Number(strategy.entryBandLo || strategy.bandLo);
    const bandHi = Number(strategy.entryBandHi || strategy.bandHi);
    const direction = String(strategy.direction).toUpperCase();

    const yPrices = market.minutePricesYes || {};
    const nPrices = market.minutePricesNo || {};
    ...
    if (direction === 'UP' && mom > momThresh) {
        const entryObj = yPrices[entryMinute];
        const entry = entryObj?.last || entryObj;
        if (!entry || entry <= 0 || entry >= 1) return null;
        if (entry < bandLo || entry > bandHi) return null;
        return { direction: 'UP', entryPrice: entry, momentum: mom };
    }

    if (direction === 'DOWN' && mom < -momThresh) {
        const entryObj = nPrices[entryMinute];
        const entry = entryObj?.last || entryObj;
        if (!entry || entry <= 0 || entry >= 1) return null;
        if (entry < bandLo || entry > bandHi) return null;
        return { direction: 'DOWN', entryPrice: entry, momentum: mom };
    }

    return null;
}

function evaluateDynamicDirection(strategy, market) {
    if (!strategy.dynamicDirection) return null;
    const dd = strategy.dynamicDirection;
    const hist = market.recentResolutions || [];
    if (dd === 'streak_fade') {
        if (hist.length < 2) return null;
        const lastTwo = hist.slice(-2);
        if (lastTwo[0] === lastTwo[1]) return lastTwo[0] === 'UP' ? 'DOWN' : 'UP';
        return null;
    }
    if (dd === 'streak_follow3') {
        if (hist.length < 3) return null;
        const last3 = hist.slice(-3);
        if (last3.every(h => h === last3[0])) return last3[0];
        return null;
    }
    if (dd === 'streak_fade3') {
        if (hist.length < 3) return null;
        const last3 = hist.slice(-3);
        if (last3.every(h => h === last3[0])) return last3[0] === 'UP' ? 'DOWN' : 'UP';
        return null;
    }
    if (dd === 'alternating') {
        if (hist.length < 2) return null;
        const last2 = hist.slice(-2);
        if (last2[0] !== last2[1]) return last2[1] === 'UP' ? 'DOWN' : 'UP';
        return null;
    }
    return null;
}

function evaluateMatchDynamic(market, nowSec, timeframe) {
    const set = strategySets[timeframe.key];
    if (!set || !set.strategies || set.strategies.length === 0) return [];
    ...
    for (const strategy of set.strategies) {
        ...
        let direction;
        if (strategy.dynamicDirection) {
            direction = evaluateDynamicDirection(strategy, market);
...
module.exports = {
    ...
    getStrategySet,
    evaluateMatch,
    evaluateMatchDynamic,
    evaluateDynamicDirection,
    evaluateMomentumSignal,
    evaluatePhase3Signal,
    sortCandidates,
    getAllLoadedSets
};
