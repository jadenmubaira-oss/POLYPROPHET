...
function strategyEntrySecondMatches(strategy, entrySecond) {
    if (Number.isFinite(Number(strategy.entrySecondMin)) || Number.isFinite(Number(strategy.entrySecondMax))) {
        const min = Number.isFinite(Number(strategy.entrySecondMin)) ? Number(strategy.entrySecondMin) : -Infinity;
        const max = Number.isFinite(Number(strategy.entrySecondMax)) ? Number(strategy.entrySecondMax) : Infinity;
        return entrySecond >= min && entrySecond <= max;
    }
    return true;
}

function minEntrySecondsBeforeClose(timeframe) {
    const key = timeframe?.key;
    const configured = CONFIG?.RISK?.minEntrySecondsBeforeClose?.[key];
    if (Number.isFinite(Number(configured))) return Math.max(0, Number(configured));
    const exitWindow = CONFIG?.RISK?.preResolutionExitSeconds?.[key];
    if (Number.isFinite(Number(exitWindow))) return Math.max(0, Number(exitWindow));
    const seconds = Number(timeframe?.seconds || 0);
    if (seconds <= 300) return 45;
...
function evaluateMatch(market, nowSec, timeframe, structuralContext = null) {
    ...
    const secondsIntoEpoch = Math.max(0, Math.floor(nowSec - epoch));
    const entrySecond = Math.max(0, secondsIntoEpoch % 60);
    const secondsUntilClose = Math.max(0, Number(timeframe.seconds || 0) - secondsIntoEpoch);
    const closeBlockSeconds = minEntrySecondsBeforeClose(timeframe);
    if (closeBlockSeconds > 0 && secondsUntilClose <= closeBlockSeconds) return [];
    ...
    for (const strategy of set.strategies) {
        ...
        if (!strategyEntrySecondMatches(strategy, entrySecond)) continue;
