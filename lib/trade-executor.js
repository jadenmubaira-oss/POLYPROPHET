...
class TradeExecutor {
    ...
    _getPositionEventTime(position) {
        const candidates = [
            ...
            position?.openedAt,
            Number.isFinite(Number(position?.pendingSince)) ? Number(position.pendingSince) : null
        ];
    ...
    _mergeDuplicatePositionGroup(group) {
        ...
        const ranked = [...group].sort((a, b) => {
            const priorityDiff = this._getPositionStatusPriority(b) - this._getPositionStatusPriority(a);
            if (priorityDiff !== 0) return priorityDiff;

            const timeDiff = this._getPositionEventTime(b) - this._getPositionEventTime(a);
            if (timeDiff !== 0) return timeDiff;

            const sharesDiff = Math.max(Number(b?.originalShares || 0), Number(b?.shares || 0))
                - Math.max(Number(a?.originalShares || 0), Number(a?.shares || 0));
            if (sharesDiff !== 0) return sharesDiff;

            return Math.max(Number(b?.originalSize || 0), Number(b?.size || 0))
                - Math.max(Number(a?.originalSize || 0), Number(a?.size || 0));
        });

        const merged = { ...ranked[0] };
        const mergedStatus = String(merged?.status || '').toUpperCase();
        const mergedIsActive = ['OPEN', 'PENDING_RESOLUTION', 'SELL_PENDING'].includes(mergedStatus);

        for (const position of ranked.slice(1)) {
            if (!merged.tokenType && position?.tokenType) merged.tokenType = position.tokenType;
            if (!merged.tokenId && position?.tokenId) merged.tokenId = position.tokenId;
            if (!merged.slug && position?.slug) merged.slug = position.slug;
            if (!merged.epoch && position?.epoch) merged.epoch = position.epoch;
            if (!merged.strategy && position?.strategy) merged.strategy = position.strategy;
            if (!merged.conditionId && position?.conditionId) merged.conditionId = position.conditionId;
            if (!merged.marketUrl && position?.marketUrl) merged.marketUrl = position.marketUrl;
            if (!merged.entrySource && position?.entrySource) merged.entrySource = position.entrySource;
            if (!merged.orderBook && position?.orderBook) merged.orderBook = position.orderBook;
            if (!merged.orderID && position?.orderID) merged.orderID = position.orderID;
            if (!merged.funderAddress && position?.funderAddress) merged.funderAddress = position.funderAddress;
            if (!Number.isFinite(Number(merged.signatureType)) && Number.isFinite(Number(position?.signatureType))) {
                merged.signatureType = Number(position.signatureType);
            }
            if (!Number.isFinite(Number(merged.requestedShares)) && Number.isFinite(Number(position?.requestedShares))) {
                merged.requestedShares = Number(position.requestedShares);
            }
            if (!merged.openedAt && position?.openedAt) merged.openedAt = position.openedAt;
            if (!merged.pendingSlug && position?.pendingSlug) merged.pendingSlug = position.pendingSlug;
            if (!merged.manualRecovery && position?.manualRecovery) merged.manualRecovery = true;
            if (!merged.manualRecoveryAt && position?.manualRecoveryAt) merged.manualRecoveryAt = position.manualRecoveryAt;
            if (!merged.manualRecoveryReason && position?.manualRecoveryReason) merged.manualRecoveryReason = position.manualRecoveryReason;
            if (!merged.manualRecoveryMeta && position?.manualRecoveryMeta) merged.manualRecoveryMeta = position.manualRecoveryMeta;
            if (!merged.recoveredFromPendingBuy && position?.recoveredFromPendingBuy) merged.recoveredFromPendingBuy = true;
            if (!merged.recoveredAt && position?.recoveredAt) merged.recoveredAt = position.recoveredAt;
            if (!merged.recoveredOrderStatus && position?.recoveredOrderStatus) merged.recoveredOrderStatus = position.recoveredOrderStatus;

            const candidateOriginalShares = Math.max(0, Number(position?.originalShares || 0));
            const candidateOriginalSize = Math.max(0, Number(position?.originalSize || 0));
            if (candidateOriginalShares > Math.max(0, Number(merged.originalShares || 0))) {
                merged.originalShares = candidateOriginalShares;
            }
            if (candidateOriginalSize > Math.max(0, Number(merged.originalSize || 0))) {
    ...
    _dedupePositions() {
        ...
        for (const positionId of orderedIds) {
            ...
            deduped.push(group.length === 1 ? group[0] : this._mergeDuplicatePositionGroup(group));
        }
    ...
    buildLiveBalanceBreakdown(usdcResult = null, clobResult = null) {
        ...
        const divergenceUsdc = bothConfirmed ? Math.abs(onChainUsdc - clobCollateralUsdc) : null;
    ...
    getCachedBalanceBreakdown() {
        ...
        if (cachedSource === 'CONFIRMED_CONSERVATIVE_MIN' || cachedSource === 'CLOB_CONFIRMED_ZERO') {
    ...
    async refreshLiveBalance(force = false) {
        ...
        try {
            ...
            if (breakdown.source === 'ZERO_CONFIRMED' || breakdown.source === 'CLOB_CONFIRMED_ZERO') {
                this.cachedLiveBalance = 0;
                this.lastGoodBalance = 0;
    ...
    recordRecoveredPendingBuy(pendingBuy, actualShares, orderStatus = null) {
        ...
        const existingPosition = existingOpen?.position
            || this._findAnyPositionByOrderId(pendingBuy.orderID)
            || (pendingBuy.positionId ? this._getPosition(pendingBuy.positionId) : null);
        const existingPositionId = existingOpen?.positionId || existingPosition?.id || pendingBuy.positionId || null;
        ...
        if (existingPosition) {
            ...
            const representedShares = Math.max(0, Number(position.originalShares || position.shares || 0));
            const deltaShares = matchedShares - representedShares;
            this._hydratePositionFromPendingBuy(position, pendingBuy);
            if (!(deltaShares > 1e-6)) {
                this._markInternalBalanceEvent();
                return { recovered: false, reason: 'NO_NEW_MATCH', positionId: existingPositionId || position.id };
            }

            const deltaSize = deltaShares * effectiveFillPrice;
            const positionStatus = String(position.status || '').toUpperCase();
            const activePosition = ['OPEN', 'PENDING_RESOLUTION', 'SELL_PENDING'].includes(positionStatus);
            if (activePosition) {
            ...
            position.originalSize = Math.max(0, Number(position.originalSize || position.size || 0)) + deltaSize;
        ...
        for (const [key, pendingBuy] of pendingEntries.slice(0, Math.max(1, maxItems))) {
            ...
            try {
                ...
                const existingPositionShares = existingPosition
                    ? Math.max(0, Number(existingPosition?.originalShares || existingPosition?.shares || 0))
                    : 0;
                const representedByExistingPosition = !!existingPosition
                ...
                if (matchedShares > recordedMatchedShares + eps || (!existingPosition && matchedShares > eps)) {
    ...
    _findRecoveryQueueIndex(match = {}) {
        return this.getRecoveryQueue().findIndex((item) => {
            ...
            if (match.tokenId && item.tokenId === match.tokenId) {
                if (match.conditionId && item.conditionId && item.conditionId !== match.conditionId) return false;
                return true;
            }
            return false;
        });
    ...
    clearRecoveryRecord(match = {}, resolution = {}) {
        ...
        if (pos) {
            ...
            pos.manualRecoveryResolvedReason = resolution?.reason || resolution?.outcome || 'RECOVERY_RESOLVED';
    ...
    async checkAndRedeemPositions() {
        ...
        for (const recovery of this.getRecoveryQueue()) {
            if (!recovery?.redeemQueued || recovery?.zeroVerified || !recovery?.tokenId) continue;
                ...
                if (balance?.success && balanceNum > 0) {
                    ...
                    if (item.conditionId) {
                        ...
                        if (redeem?.success) {
                            ...
                            this.clearRecoveryRecord({ positionId: item.positionId || null, tokenId: item.tokenId, conditionId: item.conditionId }, {
                                outcome: 'REDEEMED',
    ...
    resetMonitoringBaseline(balance, options = {}) {
        ...
        this.baselineBankrollInitialized = true;
        this.baselineBankrollSource = String(options.source || 'manual_monitoring_reset');
