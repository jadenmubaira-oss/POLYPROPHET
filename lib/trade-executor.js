...
class TradeExecutor {
    ...
    async executeTrade(candidate, market) {
        ...
        const effectiveMinOrderShares = Math.max(CONFIG.RISK.minOrderShares, marketMinOrderShares);
        const runtimeCandidate = { ...candidate, epoch: market.epoch, minOrderShares: effectiveMinOrderShares };
        ...
        // Check risk gates
        const riskCheck = this.risk.canTrade(runtimeCandidate);
        if (!riskCheck.allowed) {
            return { success: false, error: riskCheck.reasons.join(', '), blocked: true };
        }

        // Calculate position size
        const sizing = this.risk.calculateSize(runtimeCandidate);
        if (sizing.blocked) {
        ...
        console.log(`🎯 Entry: ${(entryPrice * 100).toFixed(1)}¢ | Shares: ${shares} | MinShares: ${effectiveMinOrderShares} | Size: $${size.toFixed(2)}`);
    ...
    check4hEmergencyExit(currentMarkets) {
        ...
        for (const pos of this.positions) {
            ...
            if (!market) continue;
            const currentPrice = pos.direction === 'UP' ? Number(market.yesPrice || 0) : Number(market.noPrice || 0);
