...
function parseLooseHitSleeveOrders(raw) {
  // Fly/PowerShell-safe fallback format observed in env:
  //   [{slug:foo,side:NO,maxPrice:0.41,stakeFraction:0.25},{...}]
  // This is NOT valid JSON (no quotes), so JSON.parse fails.
  const text = String(raw || '').trim();
  if (!text) return [];
  if (!text.includes('slug:') || !text.includes('side:')) return [];

  const objects = text.match(/\{[^}]*\}/g) || [];
  const out = [];
  for (const obj of objects) {
    const slug = (obj.match(/\bslug:([^,}]+)/)?.[1] || '').trim();
    const side = (obj.match(/\bside:([^,}]+)/)?.[1] || '').trim();
    const maxPriceRaw = (obj.match(/\bmaxPrice:([^,}]+)/)?.[1] || '').trim();
    const stakeFractionRaw = (obj.match(/\bstakeFraction:([^,}]+)/)?.[1] || '').trim();

    const maxPrice = Number(maxPriceRaw);
    const stakeFraction = Number(stakeFractionRaw);
...
function loadHitSleeveOrders() {
  ...
  const orders = Array.isArray(parsed) ? parsed : parseLooseHitSleeveOrders(raw);
  const rawCount = Array.isArray(parsed) ? parsed.length : (Array.isArray(orders) ? orders.length : 0);
