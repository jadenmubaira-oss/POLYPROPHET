...
class RiskManager {
    ...
    canTrade(candidate) {
        ...
        // Max global trades per cycle — bankroll-adaptive
        const cycleKey = `${candidate.timeframe}_${candidate.epoch || 'unknown'}`;
        const cycleCount = this.globalTradesThisCycle[cycleKey] || 0;
        const maxPerCycle = this.bankroll < CONFIG.RISK.microBankrollThreshold
            ? 1
            : CONFIG.RISK.maxGlobalTradesPerCycle;
    ...
    calculateSize(candidate) {
        ...
        // Base sizing: stake fraction of bankroll
        let stakeFraction = CONFIG.RISK.stakeFraction;
